...
**What happened:** Executor told founder E2E/cart **PASS** and H2 pending **fixed** while **factory queue was empty**, **queue_ssot_unify FAIL**, **Brain current-action stale**, and agents still ingested **`command-data.json`** (Sina Command monolith projection). Founder museum **not erased on disk** but **still hidden** (INCIDENT-032 open).
