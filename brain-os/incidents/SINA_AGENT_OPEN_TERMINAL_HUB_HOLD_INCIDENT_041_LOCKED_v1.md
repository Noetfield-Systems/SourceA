...
> **Agents MUST NOT hold open Cursor/shell terminals with long-running servers. Founder never Terminal. Hub boot = one-shot script or launchctl — script exits, terminal closes. If Hub dies after Ready, fix launchctl/nohup on disk — do NOT pin a terminal open.**
...
- **If Hub up:** one `curl` POST to `/api/fbe/forge--run/v1` (dry_run) · update proof receipt · **STOP**.
