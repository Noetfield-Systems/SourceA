...
check "${ROOT}/brain-os/contract/MANDATORY_BRAIN_CHAT_LOCKED_v1.md" "brain-session-start.sh"
check "${ROOT}/brain-os/contract/BRAIN_COMPLETE_TRANSFER_LOCKED_v1.md" "BRAIN_DISK_BEFORE_CHAT_SESSION_LOOP"
check "${ROOT}/brain-os/law/BRAIN_RULES_AUTHORITY_INDEX_LOCKED_v1.md" "Brain disk-before-chat"
check "${ROOT}/brain-os/scripts/sync-brain-pack.sh" "BRAIN_DISK_BEFORE_CHAT_SESSION_LOOP"
check "${ROOT}/.cursor/rules/000-entry-gate.mdc" "brain-session-start.sh"

bash "$ROOT/brain-os/scripts/validate-cursor-entry-gate-v1.sh" || errors=$((errors + 1))
