# sa-mkt-0721 — Activepieces · Run history & proof
...
From https://www.activepieces.com document Activepieces run/history UX: Build flows; trigger on events; view run history — map to `Worker job run detail page (pass/fail/steps/logs/retry)`

## Implementation extraction

`Activepieces · Run history & proof` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0721`
