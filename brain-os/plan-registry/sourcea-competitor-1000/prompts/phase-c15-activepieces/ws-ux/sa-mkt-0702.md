# sa-mkt-0702 — Activepieces · Buyer-visible UX
...
Write one-line UX spec: `Activepieces Buyer-visible UX` → buyer sees X → we show on `Worker Hub Next steps + factory-now line` (lesson: Self-hostable workflow runs for portfolio factories)
...
1. `status: done` in REGISTRY.json for `sa-mkt-0702`
