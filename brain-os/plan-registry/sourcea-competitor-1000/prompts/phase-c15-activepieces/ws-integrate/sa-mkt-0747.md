# sa-mkt-0747 — Activepieces · Integrations & API
...
Rate-limit + retry policy copied from Activepieces operating model (Build flows; trigger on events; view run history)

## Implementation extraction

`Activepieces · Integrations & API` → what buyer sees at vendor → what we ship logged with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0747`
