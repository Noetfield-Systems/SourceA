# sa-mkt-0047 — Trigger.dev · Integrations & API
...
Rate-limit + retry policy copied from Trigger.dev operating model (Developer writes tasks in TypeScript; platform executes, logs steps, retries failures)

## Implementation extraction

`Trigger.dev · Integrations & API` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0047`
