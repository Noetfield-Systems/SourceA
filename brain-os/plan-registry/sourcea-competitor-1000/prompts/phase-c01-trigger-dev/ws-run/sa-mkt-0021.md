# sa-mkt-0021 — Trigger.dev · Run history & proof
...
From https://trigger.dev · https://trigger.dev/pricing document Trigger.dev run/history UX: Developer writes tasks in TypeScript; platform executes, logs steps, retries failures — map to `Worker job run detail page (pass/fail/steps/logs/retry)`

## Implementation extraction

`Trigger.dev · Run history & proof` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0021`
