# sa-mkt-0031 — Trigger.dev · Onboarding & PLG
...
Document Trigger.dev onboarding path (Open source → dev community → PLG cloud tiers): who runs (Vendor-managed cloud; optional self-host MIT) → first value moment

## Implementation extraction

`Trigger.dev · Onboarding & PLG` → what buyer sees at vendor → what we ship logged with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0031`
