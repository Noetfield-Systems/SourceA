# sa-mkt-0002 — Trigger.dev · Buyer-visible UX
...
Write one-line UX spec: `Trigger.dev Buyer-visible UX` → buyer sees X → we show on `Worker Hub Next steps + factory-now line` (lesson: Run detail page with pass/fail, steps, logs, retry — copy for Worker Hub job dashboard)
...
1. `status: done` in REGISTRY.json for `sa-mkt-0002`
