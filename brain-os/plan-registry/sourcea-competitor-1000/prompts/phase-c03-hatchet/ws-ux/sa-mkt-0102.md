# sa-mkt-0102 — Hatchet · Buyer-visible UX
...
Write one-line UX spec: `Hatchet Buyer-visible UX` → buyer sees X → we show on `Worker Hub Next steps + factory-now line` (lesson: Priority lanes + durable steps for agent orchestration)
...
1. `status: done` in REGISTRY.json for `sa-mkt-0102`
