# sa-mkt-0121 — Hatchet · Run history & proof
...
From https://hatchet.run document Hatchet run/history UX: Postgres-backed task DAG; workers execute with priority lanes — map to `Worker job run detail page (pass/fail/steps/logs/retry)`

## Implementation extraction

`Hatchet · Run history & proof` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0121`
