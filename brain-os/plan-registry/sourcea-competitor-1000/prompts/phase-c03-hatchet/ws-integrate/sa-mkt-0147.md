# sa-mkt-0147 — Hatchet · Integrations & API
...
Rate-limit + retry policy copied from Hatchet operating model (Postgres-backed task DAG; workers execute with priority lanes)

## Implementation extraction

`Hatchet · Integrations & API` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0147`
