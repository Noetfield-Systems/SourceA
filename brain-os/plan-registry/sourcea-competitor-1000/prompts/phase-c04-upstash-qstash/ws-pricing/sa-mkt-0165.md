# sa-mkt-0165 — Upstash QStash · Pricing & packaging
...
Compare Upstash QStash PLG motion (Developer PLG via Upstash ecosystem) vs our onboarding — one adoption fix on `Hub Actions + RUN INBOX first-run path`

## Implementation extraction

`Upstash QStash · Pricing & packaging` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0165`
