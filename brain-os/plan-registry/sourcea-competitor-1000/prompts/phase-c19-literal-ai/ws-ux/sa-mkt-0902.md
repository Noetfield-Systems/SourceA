# sa-mkt-0902 — Literal AI · Buyer-visible UX
...
Write one-line UX spec: `Literal AI Buyer-visible UX` → buyer sees X → we show on `Worker Hub Next steps + factory-now line` (lesson: Conversation/run logging for agent sessions)
...
1. `status: done` in REGISTRY.json for `sa-mkt-0902`
