# sa-mkt-0247 — Langfuse · Integrations & API
...
Rate-limit + retry policy copied from Langfuse operating model (SDK captures traces; dashboard filters by user, session, cost, latency)

## Implementation extraction

`Langfuse · Integrations & API` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0247`
