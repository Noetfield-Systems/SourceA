# sa-mkt-0897 — Portkey · Integrations & API
...
Rate-limit + retry policy copied from Portkey operating model (Proxy all LLM calls; log, route, cache)

## Implementation extraction

`Portkey · Integrations & API` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0897`
