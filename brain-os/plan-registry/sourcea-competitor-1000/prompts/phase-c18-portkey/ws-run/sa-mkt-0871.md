# sa-mkt-0871 — Portkey · Run history & proof
...
From https://portkey.ai document Portkey run/history UX: Proxy all LLM calls; log, route, cache — map to `Worker job run detail page (pass/fail/steps/logs/retry)`

## Implementation extraction

`Portkey · Run history & proof` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0871`
