# sa-mkt-0865 — Portkey · Pricing & packaging
...
Compare Portkey PLG motion (Gateway wedge in crowded LLM ops) vs our onboarding — one adoption fix on `Hub Actions + RUN INBOX first-run path`

## Implementation extraction

`Portkey · Pricing & packaging` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0865`
