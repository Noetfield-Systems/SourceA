# sa-mkt-0452 — Giskard · Buyer-visible UX
...
Write one-line UX spec: `Giskard Buyer-visible UX` → buyer sees X → we show on `Worker Hub Next steps + factory-now line` (lesson: Pre-prod agent regression test library)
...
1. `status: done` in REGISTRY.json for `sa-mkt-0452`
