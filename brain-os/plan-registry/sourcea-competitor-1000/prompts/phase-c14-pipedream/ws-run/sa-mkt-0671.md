# sa-mkt-0671 — Pipedream · Run history & proof
...
From https://pipedream.com document Pipedream run/history UX: Visual + code workflows; each run logged — map to `Worker job run detail page (pass/fail/steps/logs/retry)`

## Implementation extraction

`Pipedream · Run history & proof` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0671`
