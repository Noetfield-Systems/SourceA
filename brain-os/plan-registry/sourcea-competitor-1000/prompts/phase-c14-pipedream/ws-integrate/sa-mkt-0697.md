# sa-mkt-0697 — Pipedream · Integrations & API
...
Rate-limit + retry policy copied from Pipedream operating model (Visual + code workflows; each run logged)

## Implementation extraction

`Pipedream · Integrations & API` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0697`
