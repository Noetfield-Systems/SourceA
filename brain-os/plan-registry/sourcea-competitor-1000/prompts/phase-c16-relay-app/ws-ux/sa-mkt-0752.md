# sa-mkt-0752 — Relay.app · Buyer-visible UX
...
Write one-line UX spec: `Relay.app Buyer-visible UX` → buyer sees X → we show on `Worker Hub Next steps + factory-now line` (lesson: Approval step before ACT/implement in worker loop)
...
1. `status: done` in REGISTRY.json for `sa-mkt-0752`
