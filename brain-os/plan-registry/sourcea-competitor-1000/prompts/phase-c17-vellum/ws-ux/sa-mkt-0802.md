# sa-mkt-0802 — Vellum · Buyer-visible UX
...
Write one-line UX spec: `Vellum Buyer-visible UX` → buyer sees X → we show on `Worker Hub Next steps + factory-now line` (lesson: Versioned prompt + eval on every dispatch)
...
1. `status: done` in REGISTRY.json for `sa-mkt-0802`
