# sa-mkt-0371 — Braintrust · Run history & proof
...
From https://www.braintrust.dev document Braintrust run/history UX: Log spans; run eval suites; compare prompt versions — map to `Worker job run detail page (pass/fail/steps/logs/retry)`

## Implementation extraction

`Braintrust · Run history & proof` → what buyer sees at vendor → what we ship logged with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0371`
