# sa-mkt-0352 — Braintrust · Buyer-visible UX
...
Write one-line UX spec: `Braintrust Buyer-visible UX` → buyer sees X → we show on `Worker Hub Next steps + factory-now line` (lesson: PASS/FAIL eval score on shipped agent runs)
...
1. `status: done` in REGISTRY.json for `sa-mkt-0352`
