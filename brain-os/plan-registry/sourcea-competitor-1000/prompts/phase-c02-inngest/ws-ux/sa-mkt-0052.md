# sa-mkt-0052 — Inngest · Buyer-visible UX
...
Write one-line UX spec: `Inngest Buyer-visible UX` → buyer sees X → we show on `Worker Hub Next steps + factory-now line` (lesson: Step timeline + trace retention tiers on every factory job)
...
1. `status: done` in REGISTRY.json for `sa-mkt-0052`
