# sa-mkt-0065 — Inngest · Pricing & packaging
...
Compare Inngest PLG motion (Dev-first docs, framework integrations, free tier) vs our onboarding — one adoption fix on `Hub Actions + RUN INBOX first-run path`

## Implementation extraction

`Inngest · Pricing & packaging` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0065`
