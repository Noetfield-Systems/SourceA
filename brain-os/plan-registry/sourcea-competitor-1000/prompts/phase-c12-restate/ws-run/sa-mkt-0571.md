# sa-mkt-0571 — Restate · Run history & proof
...
From https://restate.dev document Restate run/history UX: Developer writes durable functions; engine persists state — map to `Worker job run detail page (pass/fail/steps/logs/retry)`

## Implementation extraction

`Restate · Run history & proof` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0571`
