# sa-mkt-0552 — Restate · Buyer-visible UX
...
Write one-line UX spec: `Restate Buyer-visible UX` → buyer sees X → we show on `Worker Hub Next steps + factory-now line` (lesson: Long-running worker without orchestrator ghost state)
...
1. `status: done` in REGISTRY.json for `sa-mkt-0552`
