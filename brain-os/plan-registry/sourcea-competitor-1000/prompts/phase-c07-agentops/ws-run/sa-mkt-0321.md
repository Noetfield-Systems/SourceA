# sa-mkt-0321 — AgentOps · Run history & proof
...
From https://www.agentops.ai document AgentOps run/history UX: SDK records agent steps, tools, costs; time-travel debugging UI — map to `Worker job run detail page (pass/fail/steps/logs/retry)`

## Implementation extraction

`AgentOps · Run history & proof` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0321`
