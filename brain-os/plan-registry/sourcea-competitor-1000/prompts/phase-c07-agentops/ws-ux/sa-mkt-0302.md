# sa-mkt-0302 — AgentOps · Buyer-visible UX
...
Write one-line UX spec: `AgentOps Buyer-visible UX` → buyer sees X → we show on `Worker Hub Next steps + factory-now line` (lesson: Time-travel replay for worker/agent debugging)
...
1. `status: done` in REGISTRY.json for `sa-mkt-0302`
