# sa-mkt-0297 — Helicone · Integrations & API
...
Rate-limit + retry policy copied from Helicone operating model (One-line proxy integration; logs every request)

## Implementation extraction

`Helicone · Integrations & API` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0297`
