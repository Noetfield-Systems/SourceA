# sa-mkt-0265 — Helicone · Pricing & packaging
...
Compare Helicone PLG motion (Open-source gateway → dev adoption) vs our onboarding — one adoption fix on `Hub Actions + RUN INBOX first-run path`

## Implementation extraction

`Helicone · Pricing & packaging` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0265`
