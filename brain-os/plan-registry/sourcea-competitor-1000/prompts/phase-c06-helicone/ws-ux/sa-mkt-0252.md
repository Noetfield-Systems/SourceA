# sa-mkt-0252 — Helicone · Buyer-visible UX
...
Write one-line UX spec: `Helicone Buyer-visible UX` → buyer sees X → we show on `Worker Hub Next steps + factory-now line` (lesson: Per-model cost and latency dashboard on dispatch)
...
1. `status: done` in REGISTRY.json for `sa-mkt-0252`
