# sa-mkt-0281 — Helicone · Onboarding & PLG
...
Document Helicone onboarding path (Open-source gateway → dev adoption): who runs (Managed cloud; OSS gateway option) → first value moment

## Implementation extraction

`Helicone · Onboarding & PLG` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `sa-mkt-0281`
