...
# [AGENT-AUTO-SOURCEA@2026-06-07] ac-0004 — Confirm healthy-drain-orchestrator deliver writes INBOX without Brain hijack
...
```
PLAN WITH NO ASF — ac-0004: Confirm healthy-drain-orchestrator deliver writes INBOX without Brain hijack (Critical — ship now, minimal diff, machine verify). Law: brain-os/contract/AUTOMATION_CONVERGE_PROGRAM_LOCKED_v1.md. Pre-flight: AUTOMATION_CONVERGE_PROGRAM + MANDATORY_SOURCEA_WORKER. Founder: Hub Actions only — no Terminal. Headless default: goal1_auto_loop. Post-flight: verify gate + PRIORITY row + WORKER_ROUND_REPORT → broker submit → STOP.
```
