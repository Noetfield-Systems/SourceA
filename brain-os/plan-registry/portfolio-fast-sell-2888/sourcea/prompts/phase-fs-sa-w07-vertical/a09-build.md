# fs-sa-0123 — Fast-sell plan

**Version:** 1 · **Tier:** T1 · **Phase:** LATER
**Lane:** SourceA · **Wedge:** w07-vertical · Vertical wedge (diaspora · RCIC · broker · trades)
**Action:** a09-build · Ship smallest build slice
**Comp anchor:** Trigger.dev · Langfuse
**Tier 1:** Asset B diagnostic $3K slice
**Tier 2:** DFY $3–10K + retainer
**SSOT:** `docs/PORTFOLIO_TWO_TIER_COMPETITOR_PRO_ANALYSIS_LOCKED_v1.md`

## Task

Fast-sell: SourceA · Vertical wedge (diaspora · RCIC · broker · trades) · Ship smallest build slice. Ship smallest slice that moves a buyer toward **Asset B diagnostic $3K slice** with a path to **DFY $3–10K + retainer**. Receipt on disk before done.

## Verify

```bash
python3 ~/Desktop/SourceA/scripts/sourcea_revenue_engine_crm_v1.py summary --json
```

## Closeout

1. `status: done` in REGISTRY.json for `fs-sa-0123`
2. Log economic signal if outreach closed (W3 / NW)
3. Bounded path only — no cross-lane without EDIT ALLOWED

---
agent_tag: AGENT-AUTO-SOURCEA
trigger: FAST SELL · PLAN WITH NO ASF
generator: generate_portfolio_fast_sell_2888_plans_v1.py v1
