...
Fast-sell: SourceA · Base fee + usage overage · Verify PASS receipt. Ship smallest slice that moves a buyer toward **Asset B diagnostic $3K slice** with a path to **DFY $3–10K + retainer**. Receipt on disk before done.
...
1. `status: done` in REGISTRY.json for `fs-sa-0086`
