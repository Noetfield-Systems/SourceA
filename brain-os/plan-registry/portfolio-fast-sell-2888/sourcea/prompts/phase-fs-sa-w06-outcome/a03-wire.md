...
Fast-sell: SourceA · Per-outcome pricing (ticket/meeting/resolution) · Wire SKU on landing. Ship smallest slice that moves a buyer toward **Asset B diagnostic $3K slice** with a path to **DFY $3–10K + retainer**. Receipt on disk before done.
...
1. `status: done` in REGISTRY.json for `fs-sa-0098`
