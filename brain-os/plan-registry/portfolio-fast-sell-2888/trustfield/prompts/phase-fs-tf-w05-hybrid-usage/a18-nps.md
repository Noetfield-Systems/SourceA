...
Fast-sell: TrustField · Base fee + usage overage · Ask for referral. Ship smallest slice that moves a buyer toward **FINTRAC readiness scan** with a path to **MSB program + settlement**. Receipt on disk before done.
...
1. `status: done` in REGISTRY.json for `fs-tf-0094`
