...
Fast-sell: SinaaiMonoRepo · Cloud forge run + Supabase row · Wire SKU on landing. Ship smallest slice that moves a buyer toward **Live :8000 demo URL** with a path to **White-label spine**. Receipt on disk before done.
...
1. `status: done` in REGISTRY.json for `fs-mx-0250`
