...
Fast-sell: WitnessBC · Hub one-tap Action · Add CRM / pipeline row. Ship smallest slice that moves a buyer toward **Public proof page pilot** with a path to **Civic annual + audit export**. Receipt on disk before done.
...
1. `status: done` in REGISTRY.json for `fs-wb-0232`
