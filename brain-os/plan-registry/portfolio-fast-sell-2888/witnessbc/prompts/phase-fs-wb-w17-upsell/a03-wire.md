...
Fast-sell: WitnessBC · Tier-1 → Tier-2 upsell motion · Wire SKU on landing. Ship smallest slice that moves a buyer toward **Public proof page pilot** with a path to **Civic annual + audit export**. Receipt on disk before done.
...
1. `status: done` in REGISTRY.json for `fs-wb-0307`
