...
# [AGENT-AUTO-SOURCEA@2026-06-14] sa-0024 — Strict build runs validate-eval-packet-v1b-live without SINA_EVAL_1B_LIVE=1
...
```
PLAN WITH NO ASF — SourceA prompt sa-0024. Strict build runs validate-eval-packet-v1b-live without SINA_EVAL_1B_LIVE=1 (Do now. Minimal diff. Run verify gate before close.). Pre-flight: read SINA_GPT_CLAUDE_WTM_SYNTHESIS_LOCKED_v1.md + brain-os/plan-registry/SOURCEA-PRIORITY.md. Founder law: hub Refresh/Actions/tabs only — no Terminal. No ASF as verify/progress authority — machine validators + auto_pass. Post-flight: verify gate + mark prompt done + SOURCEA-PRIORITY evidence row.
```
