...
# [AGENT-AUTO-SOURCEA@2026-06-14] sa-0257 — Add fleet scan timeout 120s in build-sina-daily-bowl refresh_fleet
...
```
PLAN WITH NO ASF — SourceA prompt sa-0257. Add fleet scan timeout 120s in build-sina-daily-bowl refresh_fleet (Quarterly hardening. No new D-modules unless plan says so.). Pre-flight: read SINA_GPT_CLAUDE_WTM_SYNTHESIS_LOCKED_v1.md + brain-os/plan-registry/SOURCEA-PRIORITY.md. Founder law: hub Refresh/Actions/tabs only — no Terminal. No ASF as verify/progress authority — machine validators + auto_pass. Post-flight: verify gate + mark prompt done + SOURCEA-PRIORITY evidence row.
```
