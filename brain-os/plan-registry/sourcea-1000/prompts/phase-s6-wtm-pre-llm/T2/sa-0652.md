...
# [AGENT-AUTO-SOURCEA@2026-06-14] sa-0652 — Cross-check LLM_CONTEXT_PACKET_SCHEMA_LOCKED_v1 vs packet builder
...
```
PLAN WITH NO ASF — SourceA prompt sa-0652. Cross-check LLM_CONTEXT_PACKET_SCHEMA_LOCKED_v1 vs packet builder (Quarterly hardening. No new D-modules unless plan says so.). Pre-flight: read SINA_GPT_CLAUDE_WTM_SYNTHESIS_LOCKED_v1.md + brain-os/plan-registry/SOURCEA-PRIORITY.md. Founder law: hub Refresh/Actions/tabs only — no Terminal. No ASF as verify/progress authority — machine validators + auto_pass. Post-flight: verify gate + mark prompt done + SOURCEA-PRIORITY evidence row.
```
