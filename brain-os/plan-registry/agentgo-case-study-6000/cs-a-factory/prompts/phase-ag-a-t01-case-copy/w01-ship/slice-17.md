# ag-a-0017 — AgentGo SA4 case study plan (v3 polished)

**Version:** 3 · **Tier:** T3 · **Phase:** NOW · **Rank:** 17
**Angle:** A · Angle A — Factory scale
**Theme:** t01-case-copy · Case study narrative copy
**Workstream:** w01-ship · Ship
**Slice:** 17/20 · **Receipt gate:** agentgo_pages_built
**SSOT:** `docs/AGENTGO_SA4_CASE_STUDY_6000_PLANS_LOCKED_v1.md`
**Inventory:** `data/agentgo-sa4-inventory-receipt-v1.json`

## Scope

AgentGo GEO factory scale — 1259-page surface · trackers · research · compare

## Task

**Document subdomain option for** case study prose (challenge · shipped · verification).

agentgo.sourcea.app — defer until ASF.

**Angle:** marketing leader who needs category surface at factory speed
**Pure law:** Pure story: controlled pipeline → 1259-page GEO surface. Not a PureFlow trades clone.
**Anti-drift:** Never claim live SaaS billing — SA4 is marketing surface proof.
**Target path:** `SourceA-landing/green-unified/case-studies/agentgo.html`
**Success metric:** buyer comprehension in one screen
**Receipt:** decision note before `status: done`.
**Phase:** NOW · **Tier:** T3 · **Workstream:** Ship

## Polish wisdom (v3)

Proof pack row optional — only if phase1 ticket still sealed logged.

## Verify

```bash
test -d ~/Desktop/SA4 && find ~/Desktop/SA4 -name '*.html' 2>/dev/null | wc -l
```

## Closeout

1. `status: done` in REGISTRY.json for `ag-a-0017`
2. Evidence row in `brain-os/plan-registry/SOURCEA-PRIORITY.md` §AgentGo case study 6000
3. No cross-lane edits without EDIT ALLOWED
4. Keep AgentGo ≠ SourceA separation in all buyer-facing copy (angle C law)

---
agent_tag: AGENT-AUTO-SOURCEA
trigger: PLAN WITH NO ASF
generator: polish_agentgo_case_study_v3.py v3
angle: cs-a-factory
smart: pure-unique-smart-polished
