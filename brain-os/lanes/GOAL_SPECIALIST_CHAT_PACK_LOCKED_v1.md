...
```text
We run a Governed Execution OS: 1 SourceA Execution Core (sole actor), 2 Goal Specialists (commercial + governance, advocate only), 1 Research Acquisitor (external facts), disk SSOT, ASF control plane. RAIS = external  reference only — not our internal layer. Running loop: Commercial → Governance → Execution Core SYNC → one sa-XXXX. Architecture locked in GOVERNED_EXECUTION_OS_MASTER_LOCKED_v1.md. No redesign — execution maturity only.
```
