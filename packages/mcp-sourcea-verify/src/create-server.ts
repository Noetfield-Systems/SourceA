...
export function createSourceaMcpServer(): McpServer {
  ...
  server.tool(
    "verify_run",
    "Return PASS/FAIL/MOCK_ONLY + receipt JSON (read-only). Does not write Mac disk.",
    {
  ...
  server.tool(
    "factory_status",
    "Read-only factory_now_line + queue head. Cloud SSE uses hub federation when configured.",
    { surface: z.enum(["hub", "brain", "inbox"]).optional() },
    async (args) => ({
  ...
  server.tool(
    "form_pick_parse",
    "Parse ASF FIVE-STEP PICK block into structured decision JSON.",
    { raw_pick: z.string() },
    async (args) => ({
  ...
  server.tool(
    "emit_receipt_readonly",
    "Validate receipt schema only — never writes ~/.sina.",
