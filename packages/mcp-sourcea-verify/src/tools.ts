...
import { RECEIPT_SCHEMA, receiptId, verdictFromReceipt, type ReceiptInput } from "./schemas.js";

export function verifyRun(input: { receipt_id?: string; receipt?: unknown; campus?: string }) {
  if (input.receipt) {
...
async function factoryStatusCloud(hubUrl: string, surface: string) {
  ...
  for (const url of urls) {
    try {
      ...
      const raw = (await res.json()) as {
        ...
        queue_sa?: string;
        updated_at?: string;
        cloud_practical_300_line?: string;
      };
      return {
        surface,
        factory_now_line:
          raw.factory_now_line ??
...
export async function factoryStatus(input: { surface?: string }) {
  const surface = input.surface ?? "hub";
  const cloud =
    process.env.SOURCEA_MCP_CLOUD === "1" ||
  ...
  try {
    const raw = JSON.parse(readFileSync(surfacesPath, "utf8")) as {
      factory_now_line?: string;
      queue_sa?: string;
      updated_at?: string;
    };
...
export function formPickParse(input: { raw_pick: string }) {
  ...
  return {
    ...
    pick: (pickMatch?.[1]?.toUpperCase() ?? "A") as "A" | "B" | "C" | "D",
    effect: effectMatch?.[1]?.trim() ?? questionMatch?.[1]?.trim() ?? "see raw_pick",
    raw_length: raw.length,
  };
}

export function emitReceiptReadonly(input: { receipt: unknown }) {
  const parsed = RECEIPT_SCHEMA.safeParse(input.receipt);
  if (!parsed.success) {
    return { schema_ok: false, violations: parsed.error.flatten() };
  }
  const receipt = parsed.data as ReceiptInput;
  const violations: string[] = [];
  if (!receipt.receipt_id && !receipt.id) violations.push("missing receipt_id or id");
  if (!receipt.verdict && !receipt.status) violations.push("missing verdict or status");
  return {
