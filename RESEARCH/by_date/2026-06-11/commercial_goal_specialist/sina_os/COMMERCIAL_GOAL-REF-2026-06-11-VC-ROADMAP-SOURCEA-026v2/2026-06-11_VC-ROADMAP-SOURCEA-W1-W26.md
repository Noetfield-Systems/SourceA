...
| **3** | **D1** demo policy P-001 + intent fixtures | Policy loads · no execute yet | Scope creep to NF product | D1 merged on disk |
...
| **6** | **D3** ALLOW stub + receipt writer | **ALLOW** → receipt JSON | No spine yet | ALLOW receipt on disk |
...
| **14** | Investor video backup + JSONL export bundle | DD export pack | Fake tamper in video | Video + JSONL on disk |
...
| **18** | Commit gate bypass audit doc (max 5 paths) | Audit "demo vs rest" | — | Audit doc on disk |
