...
for stack in sourcea witnessbc noetfield trustfield virlux; do
  ...
  if [[ "$LIVE" -eq 1 ]]; then
    python3 "$ROOT/scripts/portfolio_competitor_forge_dispatch_v1.py" --stack "$stack" --mode railway_fbe --json >/dev/null \
      || echo "WARN live dispatch $stack (cloud may be down)"
  else
    python3 "$ROOT/scripts/portfolio_competitor_forge_dispatch_v1.py" --stack "$stack" --dry-run --json >/dev/null \
