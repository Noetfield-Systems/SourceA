#!/usr/bin/env bash
# Build Portfolio Mail — true double-click .app, no Terminal, own window (WKWebView).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
APPS_BRAND="$ROOT/brand/macos-apps"
APP_NAME="Portfolio Mail"
APP_ID="com.sina.portfoliomail.standalone"
DESKTOP="$HOME/Desktop/${APP_NAME}.app"
APPS_HOME="$HOME/Applications/${APP_NAME}.app"
ENT="$APPS_BRAND/entitlements-local.plist"
ICNS="$ROOT/brand/icons/icns/sina-portfolio-mail.icns"
LSREGISTER="/System/Library/Frameworks/CoreServices.framework/Frameworks/LaunchServices.framework/Support/lsregister"
SWIFT_SRC="$APPS_BRAND/PortfolioMailShell.swift"
STAGE="$APPS_BRAND/${APP_NAME}.app"
BUNDLE_STAGE="$STAGE/Contents/Resources/portfolio-mail-bundle"
EXEC_NAME="PortfolioMailShell"
OLD_COMMAND="$HOME/Desktop/Portfolio Mail.command"
...
rm -rf "$DESKTOP" "$APPS_HOME" "$OLD_COMMAND"
ditto "$STAGE" "$DESKTOP"
...
if [[ "$COLD_OK" != "1" ]]; then
  echo "WARN: cold-start failed — see ~/.sina/portfolio-mail-app-launch.log" >&2
  tail -5 "$HOME/.sina/portfolio-mail-app-launch.log" 2>/dev/null || true
fi
...
echo "  Removed:       $OLD_COMMAND"
echo "  Log:           ~/.sina/portfolio-mail-app-launch.log"
