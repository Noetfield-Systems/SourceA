#!/usr/bin/env bash
# validate-lane-p0-revenue-bottleneck-t2-crossref-v1.sh — sa-0560 ACT T2 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0560-lane-p0-revenue-bottleneck-t2-crossref_LOCKED_v1.md"
...
assert cross.is_file(), "missing sa-0560 cross-ref doc"
...
hook = (pp.get("signals_auto") or {}).get("lane_p0_revenue_bottleneck_t2_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "t1_echo_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.lane_p0_revenue_bottleneck_t2_crossref missing {key}")
...
if "sa-0560-lane-p0-revenue-bottleneck-t2-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0560 attachment")

print("OK: validate-lane-p0-revenue-bottleneck-t2-crossref-v1 · canonical=sa-0510 · t1=sa-0535 · sa-0560")
PY
