...
def _source_url(plan: dict[str, Any]) -> str:
    ...
    if from_action:
        return from_action
    competitor = str(plan.get("competitor") or "")
    workstream = str(plan.get("workstream") or "ws-ux")
    urls = COMPETITOR_URLS.get(competitor) or {}
...
def _vendor_says_snippets(plan: dict[str, Any]) -> list[str]:
    """When competitor blocks bot fetch, use the vendor claim already locked in cloud_action."""
    action = str(plan.get("cloud_action") or "")
...
def _candidate_urls(plan: dict[str, Any]) -> list[str]:
    ...
    competitor = str(plan.get("competitor") or "")
    workstream = str(plan.get("workstream") or "ws-ux")
    cmap = COMPETITOR_URLS.get(competitor) or {}
...
def _run_cloud_plan(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    show = (
        f"{plan.get('id')} · {plan.get('competitor')} · {plan.get('workstream')} — "
...
def _gap_line(receipt: dict[str, Any]) -> str:
    competitor = str(receipt.get("competitor") or "")
    area = _area(str(receipt.get("workstream") or ""))
    action = str(receipt.get("cloud_action") or "")
    snippet = _best_snippet(receipt)
    if snippet:
        return f"{competitor} {area}: {action} — competitor evidence: {snippet}"
    return f"{competitor} {area}: {action}"
...
def build_summary() -> dict[str, Any]:
    ...
    for receipt in receipts:
        if receipt.get("status") != "PASS" and not receipt.get("ok"):
            continue
        competitor = str(receipt.get("competitor") or "")
        if competitor not in findings:
            findings[competitor] = {a: {"plans": [], "evidence": [], "actions": []} for a in WORKSTREAM_AREA.values()}
        area = _area(str(receipt.get("workstream") or ""))
        bucket = findings[competitor].setdefault(area, {"plans": [], "evidence": [], "actions": []})
    ...
    for cand in gap_candidates:
        key = f"{cand.get('competitor')}:{cand.get('area')}:{cand.get('gap')[:80]}"
        if key in seen:
            continue
        seen.add(key)
    ...
    summary = {
        ...
        "findings_by_competitor": findings,
        "top_10_gaps": [
            {
                "rank": g["rank"],
                "gap": g["gap"],
