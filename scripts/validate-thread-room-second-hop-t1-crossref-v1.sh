#!/usr/bin/env bash
# validate-thread-room-second-hop-t1-crossref-v1.sh — sa-0844 T1 dedup cross-ref → canonical sa-0819
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT/scripts"

fail() { echo "FAIL: validate-thread-room-second-hop-t1-crossref-v1 — $*" >&2; exit 1; }

CROSS="$ROOT/archive/attachments/2026-06-15/sa-0844-thread-room-second-hop-t1-crossref_LOCKED_v1.md"
CANONICAL="$ROOT/scripts/validate-thread-room-second-hop-v1.sh"
RECEIPT="$ROOT/receipts/sa-0819-receipt.json"

[[ -f "$CROSS" ]] || fail "missing sa-0844 cross-ref doc"
...
cross = Path("../archive/attachments/2026-06-15/sa-0844-thread-room-second-hop-t1-crossref_LOCKED_v1.md")
...
print("OK: sa-0844 cross-ref doc cites sa-0819 canonical")
PY

bash "$CANONICAL" >/dev/null || fail "canonical validate-thread-room-second-hop-v1"

echo "OK: validate-thread-room-second-hop-t1-crossref-v1 · canonical=sa-0819 · sa-0844"
