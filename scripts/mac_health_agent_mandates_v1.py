...
def _ensure_notifications_off() -> dict[str, Any]:
    try:
        ...
        from mac_health_settings_v1 import load_settings, save_settings  # noqa: WPS433

        cur = load_settings()
        notes = cur.get("notifications") or {}
        if notes.get("enabled") or notes.get("sounds_enabled"):
