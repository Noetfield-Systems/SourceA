...
def _cycle_proof_slice(cycle_doc: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "competitor": dispatch.get("competitor") or forge.get("competitor"),
...
def evidence_audit_html(*, limit: int = 50, offset: int = 0, plan_id: str = "") -> str:
    ...
    for r in payload.get("rows") or [payload.get("row")] if payload.get("row") else []:
        ...
        rows_html.append(
            ...
            f"<td>{esc(r.get('maps_registry'))}</td><td>{esc(r.get('competitor'))}</td>"
...
{detail}
<table><thead><tr><th>Plan</th><th>Tier</th><th>Registry</th><th>Competitor</th><th>HTTP</th><th>At</th><th>Snippet</th></tr></thead>
...
<div class=now-grid>
  <div class=box>
    ...
    <div>{esc(head_plan.get('maps_registry'))} · {esc(head_plan.get('competitor'))} · {esc(head_plan.get('tier'))}</div>
...
<div class=box style="margin-top:12px">
  <div class=label>Evidence snippets (from competitor page fetch)</div>
