#!/usr/bin/env bash
# validate-evidence-flywheel-hub-essentials-t3-crossref-v1.sh — sa-0594 ACT T3 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0594-evidence-flywheel-hub-essentials-t3-crossref_LOCKED_v1.md"
...
assert cross.is_file(), "missing sa-0594 cross-ref doc"
...
hook = (pp.get("signals_auto") or {}).get("evidence_flywheel_hub_t3_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "t1_echo_sa", "t2_echo_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.evidence_flywheel_hub_t3_crossref missing {key}")
...
if hook.get("t2_echo_sa") != "sa-0569":
    raise SystemExit("FAIL: t2_echo_sa must be sa-0569")
if "sa-0594-evidence-flywheel-hub-essentials-t3-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0594 attachment")

print("OK: validate-evidence-flywheel-hub-essentials-t3-crossref-v1 · canonical=sa-0519 · t2=sa-0569 · sa-0594")
PY
