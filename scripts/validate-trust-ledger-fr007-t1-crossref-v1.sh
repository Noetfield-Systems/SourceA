#!/usr/bin/env bash
# validate-trust-ledger-fr007-t1-crossref-v1.sh — sa-0528 ACT T1 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0528-trust-ledger-fr007-t1-crossref_LOCKED_v1.md"
...
assert cross.is_file(), "missing sa-0528 cross-ref doc"
text = cross.read_text(encoding="utf-8")
for needle in ("sa-0503", "sa-0528", "sa-0503-trust-ledger-fr007_LOCKED_v1.md", "TRUST_LEDGER_SCHEMA_LOCKED_v1.md"):
...
hook = (pp.get("signals_auto") or {}).get("trust_ledger_fr007_t1_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.trust_ledger_fr007_t1_crossref missing {key}")
if hook.get("canonical_sa") != "sa-0503":
    raise SystemExit("FAIL: canonical_sa must be sa-0503")
if "sa-0528-trust-ledger-fr007-t1-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0528 attachment")

print("OK: validate-trust-ledger-fr007-t1-crossref-v1 · canonical=sa-0503 · sa-0528")
PY
