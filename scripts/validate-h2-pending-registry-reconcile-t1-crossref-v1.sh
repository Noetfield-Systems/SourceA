#!/usr/bin/env bash
# validate-h2-pending-registry-reconcile-t1-crossref-v1.sh — sa-0838 T1 dedup cross-ref → canonical sa-0813
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT/scripts"

fail() { echo "FAIL: validate-h2-pending-registry-reconcile-t1-crossref-v1 — $*" >&2; exit 1; }

CROSS="$ROOT/archive/attachments/2026-06-15/sa-0838-h2-pending-registry-reconcile-t1-crossref_LOCKED_v1.md"
CANONICAL="$ROOT/scripts/validate-h2-pending-registry-reconcile-v1.sh"
RECEIPT="$ROOT/receipts/sa-0813-receipt.json"

[[ -f "$CROSS" ]] || fail "missing sa-0838 cross-ref doc"
...
cross = Path("../archive/attachments/2026-06-15/sa-0838-h2-pending-registry-reconcile-t1-crossref_LOCKED_v1.md")
...
print("OK: sa-0838 cross-ref doc cites sa-0813 canonical")
PY

bash "$CANONICAL" >/dev/null || fail "canonical validate-h2-pending-registry-reconcile-v1"

echo "OK: validate-h2-pending-registry-reconcile-t1-crossref-v1 · canonical=sa-0813 · sa-0838"
