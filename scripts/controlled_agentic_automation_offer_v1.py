#!/usr/bin/env python3
"""Asset B — controlled agentic automation DFY offer receipt + outreach pack."""
...
SKUS: list[dict[str, Any]] = [
    {
        ...
        "deliverable": "One controlled agent loop live · handoff doc · 30-day fix window",
    },
    {
        "id": "SKU-RET-001",
    ...
    {
        "id": "SKU-COMBO-001",
        "name": "Controlled Automation + Receipts",
        "price_min_usd": 3000,
        "price_max_usd": 15000,
        "type": "combo",
        "timeline_weeks": "2-4",
...
OUTREACH_TARGETS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "priority": "P0",
        "sku": "SKU-RET-001",
        "snippet": "$2K/mo — tracked outreach or ops loop with weekly receipt export your stakeholders can forward.",
    },
    {
        "id": "AB-T03",
...
EMAIL_TEMPLATES_BODY: dict[str, Any] = {
    ...
    "ab1_short_punchy": {
        ...
        "body": (
            ...
            "We ship controlled agent loops with signed receipts — $3–10K build or $2–5K/mo retainer. "
    ...
    "ab1_chain_tool": {
        ...
        "body": (
            ...
            "We also build controlled agent loops for teams that want done-for-you delivery. "
...
def build_receipt(*, refresh_gate: bool = False) -> dict[str, Any]:
    ...
    w3_log = _read_json(SINA / "controlled-agentic-automation-w3-v1.json", {"events": []})
    ...
    return {
        "version": "1.0",
        "asset": "B",
        "law": "SOURCEA_ASSET_B_CONTROLLED_AGENTIC_AUTOMATION_LOCKED_v1.md",
        "updated_at": _now_iso(),
        ...
        "skus": SKUS,
        "speed_to_cash_rank": [
            {"rank": 1, "motion": "DFY controlled agentic automation", "timeline": "weeks"},
...
def log_w3(sku: str, amount_usd: float, *, client: str = "", case_study: bool = False) -> dict[str, Any]:
    path = SINA / "controlled-agentic-automation-w3-v1.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Asset B controlled agentic automation offer")
