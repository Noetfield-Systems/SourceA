...
def workstream_tasks(workstream: str, *, pick: dict[str, Any], run_id: str) -> list[dict[str, Any]]:
    ...
    ws_route = (rules.get("route_by__workstream") or {}).get(workstream, "gpt_control")
    kind_map = {
        ...
        tasks.append(
            {
                "id": tid,
                "kind": kind,
                "title": f"{pick.get('', '?')} · {workstream} · {kind}",
...
def build_task_graph(*, pick: dict[str, Any], run_id: str | None = None) -> dict[str, Any]:
    ...
    title = str(pick.get("title") or pick.get("") or "")[:500]
    return {
        ...
        "prompt": {
            "raw": title,
            "intent": f"{pick.get('stack')}  parity · {pick.get('')} · {ws}",
            "constraints": ["cloud_only", "receipt_required", "mac_build_forbidden"],
            "entities": [str(pick.get("") or ""), ws],
            "risk_signals": [],
        },
        "tasks": tasks,
        "edges": edges,
