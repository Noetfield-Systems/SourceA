#!/usr/bin/env bash
# validate-mergepack-kpi-trio-t1-crossref-v1.sh — sa-0534 ACT T1 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0534-mergepack-kpi-trio-t1-crossref_LOCKED_v1.md"
canonical_doc = root / "archive/attachments/2026-06-14/sa-0509-mergepack-kpi-trio_LOCKED_v1.md"
command_center_law = root / "ASF_PROGRAM_PROGRESS_COMMAND_CENTER_LOCKED_v1.md"
receipt = root / "receipts/sa-0509-receipt.json"

assert cross.is_file(), "missing sa-0534 cross-ref doc"
...
assert command_center_law.is_file(), "ASF_PROGRAM_PROGRESS_COMMAND_CENTER_LOCKED_v1.md missing"
...
hook = (pp.get("signals_auto") or {}).get("mergepack_kpi_trio_t1_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.mergepack_kpi_trio_t1_crossref missing {key}")
if hook.get("canonical_sa") != "sa-0509":
    raise SystemExit("FAIL: canonical_sa must be sa-0509")
if "sa-0534-mergepack-kpi-trio-t1-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0534 attachment")

print("OK: validate-mergepack-kpi-trio-t1-crossref-v1 · canonical=sa-0509 · sa-0534")
PY
