#!/usr/bin/env bash
# validate-controlled-agentic-automation-offer-v1.sh — Asset B commercial lane wired
set -euo pipefail
...
fail() { echo "FAIL: validate-controlled-agentic-automation-offer-v1 — $*" >&2; exit 1; }

LAW="$ROOT/SOURCEA_ASSET_B_CONTROLLED_AGENTIC_AUTOMATION_LOCKED_v1.md"
SSOT="$SOURCEA_UNIFIED_PORTFOLIO_COMMERCIAL_SSOT"
AGENCY="$ROOT/SOURCEA_AGENCY_PRODUCT_DEMO_SCRIPT_LOCKED_v1.md"
ENGINE="$ROOT/scripts/controlled_agentic_automation_offer_v1.py"
...
law = Path("SOURCEA_ASSET_B_CONTROLLED_AGENTIC_AUTOMATION_LOCKED_v1.md").read_text(encoding="utf-8")
required = (
    "LOCKED v1",
    "Asset B",
    "SKU-DFY-001",
    ...
    "Combined motion",
    "AB1",
    "controlled_agentic_automation_offer_v1.py",
    "validate-controlled-agentic-automation-offer-v1.sh",
    "~$200/mo",
)
...
grep -q "SOURCEA_ASSET_B_CONTROLLED_AGENTIC_AUTOMATION_LOCKED_v1.md" "$SSOT" || fail "SSOT missing Asset B law pointer"
...
import json
from pathlib import Path
p = Path.home() / ".sina/controlled-agentic-automation-email-templates-v1.json"
...
python3 scripts/controlled_agentic_automation_offer_v1.py --pack --json >/dev/null || fail "offer pack failed"

RECEIPT="${HOME}/.sina/controlled-agentic-automation-offer-v1.json"
...
data = json.loads(Path.home().joinpath(".sina/controlled-agentic-automation-offer-v1.json").read_text())
assert data.get("asset") == "B"
assert data.get("law", "").endswith("SOURCEA_ASSET_B_CONTROLLED_AGENTIC_AUTOMATION_LOCKED_v1.md")
...
echo "OK: validate-controlled-agentic-automation-offer-v1 · Asset B lane wired"
