#!/usr/bin/env bash
# validate-founder-request-build-sync-t2-crossref-v1.sh — sa-0557 ACT T2 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0557-founder-request-build-sync-t2-crossref_LOCKED_v1.md"
...
assert cross.is_file(), "missing sa-0557 cross-ref doc"
...
hook = (pp.get("signals_auto") or {}).get("founder_request_build_sync_t2_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "t1_echo_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.founder_request_build_sync_t2_crossref missing {key}")
...
if "sa-0557-founder-request-build-sync-t2-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0557 attachment")

print("OK: validate-founder-request-build-sync-t2-crossref-v1 · canonical=sa-0507 · t1=sa-0532 · sa-0557")
PY
