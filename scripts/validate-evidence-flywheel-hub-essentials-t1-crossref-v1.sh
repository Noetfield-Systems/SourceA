#!/usr/bin/env bash
# validate-evidence-flywheel-hub-essentials-t1-crossref-v1.sh — sa-0544 ACT T1 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0544-evidence-flywheel-hub-essentials-t1-crossref_LOCKED_v1.md"
...
assert cross.is_file(), "missing sa-0544 cross-ref doc"
...
hook = (pp.get("signals_auto") or {}).get("evidence_flywheel_hub_t1_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.evidence_flywheel_hub_t1_crossref missing {key}")
if hook.get("canonical_sa") != "sa-0519":
    raise SystemExit("FAIL: canonical_sa must be sa-0519")
if "sa-0544-evidence-flywheel-hub-essentials-t1-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0544 attachment")

print("OK: validate-evidence-flywheel-hub-essentials-t1-crossref-v1 · canonical=sa-0519 · sa-0544")
PY
