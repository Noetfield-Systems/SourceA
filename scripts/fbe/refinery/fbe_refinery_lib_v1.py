"""FBE refinery lib — CREED read-only delegate, bay receipts, stage ledger."""
...
NPM_MAP: dict[str, str] = {
    ...
    "factory-definition-v1": "check:factory-definition",
    "factory-mirror-v1": "creed:mirror",
    "factory-route-audit-v1": "creed:route-audit",
    "factory-clone-parity-v1": "creed:clone-parity",
...
def bay_dir(bay_slug: str) -> Path:
    return ROOT / "receipts" / "bays" / bay_slug


def ledger_path(bay_slug: str) -> Path:
    return bay_dir(bay_slug) / "ledger.jsonl"


def receipt_path(bay_slug: str, node_id: str) -> Path:
    safe = node_id.replace("/", "_")
    return bay_dir(bay_slug) / f"{safe}-v1.json"
...
def run_creed_npm(
    *,
    ...
    env = {**os.environ, "CREED_PROJECT": bay_slug, "CREED_ROOT": str(creed)}
    t0 = time.monotonic()
    try:
        out = subprocess.run(
            cmd,
...
def wrapper_main(node_id: str) -> int:
    ...
    args = ap.parse_args()
    row = run_creed_npm(node_id=node_id, bay_slug=args.bay, tenant=args.tenant)
    if args.json:
