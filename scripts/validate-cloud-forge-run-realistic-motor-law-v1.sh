#!/usr/bin/env bash
# validate-cloud-forge-run-realistic-motor-law-v1.sh — INCIDENT-045 law logged
set -euo pipefail
...
motor = ssot.get('motor') or {}
cap = int(motor.get('rows_per_turn_cap') or ssot.get('rows_per_turn_cap') or 0)
assert 1 <= cap <= 10, cap
assert int(motor.get('max_advance_per_tick') or ssot.get('max_advance_per_tick') or 0) == cap
assert ssot.get('proof', {}).get('proof_gate_required') is True
assert int(rt.get('max_advance_per_tick') or 0) == cap
assert int(rt.get('rows_per_turn_cap') or 0) == cap
...
from pathlib import Path
ssot = json.loads(Path('$SSOT').read_text())
cap = int((ssot.get('motor') or {}).get('rows_per_turn_cap') or ssot.get('rows_per_turn_cap') or 0)
fp = json.loads(Path('$FP').read_text())
assert int(fp.get('pattern', {}).get('max_advance_per_tick') or fp.get('max_advance_per_tick') or 0) == cap
