#!/usr/bin/env bash
# Arm Cloudflare cloud-auto-runtime-tick-v1 for 24/7 auto Proceed (founder-run once).
set -euo pipefail
...
echo "→ CLOUD_FORGE_RUN_AUTO_PROCEED=true"
printf '%s' "true" | npx wrangler secret put CLOUD_FORGE_RUN_AUTO_PROCEED

echo ""
echo "OK — 24/7 armed. Test:"
echo "  curl -s https://sourcea-cloud-auto-runtime-tick-v1.sina-kazemnezhad-ca.workers.dev/health"
echo "  curl -s -X POST https://sourcea-cloud-auto-runtime-tick-v1.sina-kazemnezhad-ca.workers.dev/tick -H 'Content-Type: application/json' -d '{\"proceed\":true}'"
