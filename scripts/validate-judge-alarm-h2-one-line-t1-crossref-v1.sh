#!/usr/bin/env bash
# validate-judge-alarm-h2-one-line-t1-crossref-v1.sh — sa-0835 T1 dedup cross-ref → canonical sa-0810
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT/scripts"

fail() { echo "FAIL: validate-judge-alarm-h2-one-line-t1-crossref-v1 — $*" >&2; exit 1; }

CROSS="$ROOT/archive/attachments/2026-06-15/sa-0835-judge-alarm-h2-one-line-t1-crossref_LOCKED_v1.md"
CANONICAL="$ROOT/scripts/validate-judge-alarm-h2-one-line-v1.sh"
RECEIPT="$ROOT/receipts/sa-0810-receipt.json"

[[ -f "$CROSS" ]] || fail "missing sa-0835 cross-ref doc"
...
cross = Path("../archive/attachments/2026-06-15/sa-0835-judge-alarm-h2-one-line-t1-crossref_LOCKED_v1.md")
...
print("OK: sa-0835 cross-ref doc cites sa-0810 canonical")
PY

bash "$CANONICAL" >/dev/null || fail "canonical validate-judge-alarm-h2-one-line-v1"

echo "OK: validate-judge-alarm-h2-one-line-t1-crossref-v1 · canonical=sa-0810 · sa-0835"
