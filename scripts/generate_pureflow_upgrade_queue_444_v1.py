#!/usr/bin/env python3
"""Generate 444 Pure Flow upgrade plans from Vancouver competitor intelligence SSOT."""
...
INTEL = ROOT / "docs/pure-flow-competitive-intelligence-vancouver-v1.md"
...
UPGRADE_PILLARS = [
    ("booking", "Booking & schedule UX", [
        "Add preferred-day/time slot UI pattern inspired by {competitor}",
        "Embed Calendly/Jobber booking step after form — beat {competitor} friction",
        "SMS confirm within 4hrs copy on booking success — match {competitor} reliability promise",
        "Mobile sticky Book CTA hides on #book in view — test vs {competitor} mobile",
        "Spa-only fast path on homepage — learn from {competitor}",
        "One-time service path without audit language — counter {competitor} tone",
        "Spring opening booking banner Mar–Apr — seasonal spike like {competitor}",
        "Fall closing booking banner Sep–Oct — seasonal spike like {competitor}",
    ...
    ("pricing", "Pricing transparency", [
        ...
        "Green recovery flat from $295 on hero float — Pond's competitive",
    ...
    ("report", "PureFlow Report proof", [
        ...
        "FAQ: what is in every PureFlow Report — educate like {competitor}",
    ...
    ("seo", "Hyperlocal SEO", [
        ...
        "Competitor counter-copy: why Pure Flow vs service in {hood}",
    ]),
    ...
    ("spa", "Spa & hot tub lane", [
        ...
        "Bi-weekly spa $89 competitor anchor mention in compare",
    ...
    ("convert", "Conversion & compare", [
        ...
        "Compare: vs {competitor} — one honest row each",
...
SITE: {SITE_URL}
INTEL: docs/pure-flow-competitive-intelligence-vancouver-v1.md

## Mission
{plan['title']}

## Competitor signal
**{plan.get('competitor', 'Market')}** — {plan.get('competitor_edge', 'Vancouver pool/spa benchmark')}
...
- Deploy only if founder ship window; else leave ready logged
...
def generate() -> dict:
    plans: list[dict] = []
    n = 0
    all_competitors = COMPETITORS_T1 + COMPETITORS_T2
    comp_i = 0
    hood_i = 0
    ...
    for level, (pillar_key, pillar_name, templates) in enumerate(UPGRADE_PILLARS, 1):
        for tmpl in templates:
            if n >= 444:
                break
            comp_name, comp_edge = all_competitors[comp_i % len(all_competitors)]
            hood = NEIGHBORHOODS[hood_i % len(NEIGHBORHOODS)]
            comp_i += 1
            hood_i += 1
            title = tmpl.format(competitor=comp_name, hood=hood, phone="(604) real-line")
            n += 1
            pid = f"pf-upg-{n:03d}"
            row = {
                "n": n,
                ...
                "competitor_edge": comp_edge,
                "neighborhood": hood if "{hood}" in tmpl else None,
                "title": title,
                "work_root": WORK_ROOT,
                "status": "queued",
                "maps_intel": "docs/pure-flow-competitive-intelligence-vancouver-v1.md",
            }
    ...
    # Pad to exactly 444 with competitor-specific counter-moves
    while n < 444:
        comp_name, comp_edge = all_competitors[comp_i % len(all_competitors)]
        hood = NEIGHBORHOODS[hood_i % len(NEIGHBORHOODS)]
        comp_i += 1
        ...
        row = {
            ...
            "queue_role": "act",
            "pillar": "counter",
            "pillar_name": "Competitor counter-move",
            "level": 13,
            "competitor": comp_name,
            "competitor_edge": comp_edge,
            "neighborhood": hood,
    ...
    return {
        ...
        "locked": True,
        "saved_at": _now(),
        "authority": "docs/pure-flow-competitive-intelligence-vancouver-v1.md",
        ...
        "summary": {
            ...
            "competitors_t1": len(COMPETITORS_T1),
            "competitors_t2": len(COMPETITORS_T2),
