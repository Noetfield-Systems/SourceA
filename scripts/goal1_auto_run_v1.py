#!/usr/bin/env python3
"""Run Goal 1 auto-run until queue exhausted — agent CLI per turn (real execution)."""
...
SINA = Path.home() / ".sina"
AUTO_LOCK = SINA / "goal1-auto-run-lock-v1.json"
...
def _run(n: int, timeout: int) -> int:
    ...
    from factory_spawn_gate_v1 import exit_if_spawn_blocked, turn_allowed  # noqa: WPS433
    from worker_inject_lib import inbox_status  # noqa: WPS433

    exit_if_spawn_blocked("goal1_auto_run")
    for i in range(n):
        gate = turn_allowed(caller="goal1_auto_run:turn")
        if not gate.get("ok"):
            _log_batch(f"[STOPPED_BY_FLAG] turn={i + 1} gate={gate.get('reason')}")
            return 0
        from active_now_v1 import sync_active_now_from_queue_head  # noqa: WPS433

        sync_active_now_from_queue_head()
        orch = _orch()
        st = orch.status()
        ...
        _log_batch(f"[auto-run] turn {i + 1}/{n} queue={pos_before}/{total} starting")
        proc = subprocess.run(
            [
                sys.executable,
        ...
        print(json.dumps({"auto_run_turn": i + 1, **row}), flush=True)
        st_after = _orch().status()
        pos_after = st_after.get("queue_pos") or pos_before
        _log_batch(
            f"[auto-run] turn {i + 1} done ok={row.get('ok')} "
        ...
        try:
            ...
            ch = val.get("chain") or {}
            _log_batch(
                f"[auto-run] BRAIN_VALIDATION inject={ch.get('inject')} validate={ch.get('validate')} "
        ...
        disk_advanced = pos_after > pos_before or role_after != role_before
        ...
        if row.get("ok") and not broker.get("ok"):
            print(
                json.dumps(
                    {
                        "stopped": True,
