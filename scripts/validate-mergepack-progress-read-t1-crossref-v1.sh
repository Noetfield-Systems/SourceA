#!/usr/bin/env bash
# validate-mergepack-progress-read-t1-crossref-v1.sh — sa-0548 ACT T1 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0548-mergepack-progress-read-t1-crossref_LOCKED_v1.md"
...
assert cross.is_file(), "missing sa-0548 cross-ref doc"
...
hook = (pp.get("signals_auto") or {}).get("mergepack_progress_read_t1_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.mergepack_progress_read_t1_crossref missing {key}")
if hook.get("canonical_sa") != "sa-0523":
    raise SystemExit("FAIL: canonical_sa must be sa-0523")
if "sa-0548-mergepack-progress-read-t1-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0548 attachment")

print("OK: validate-mergepack-progress-read-t1-crossref-v1 · canonical=sa-0523 · sa-0548")
PY
