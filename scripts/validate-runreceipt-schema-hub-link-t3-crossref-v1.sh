#!/usr/bin/env bash
# validate-runreceipt-schema-hub-link-t3-crossref-v1.sh — sa-0580 ACT T3 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0580-runreceipt-schema-hub-link-t3-crossref_LOCKED_v1.md"
...
assert cross.is_file(), "missing sa-0580 cross-ref doc"
...
for needle in (
    ...
    "validate-runreceipt-schema-hub-link-v1.sh",
):
...
hook = (pp.get("signals_auto") or {}).get("runreceipt_schema_hub_link_t3_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "t1_echo_sa", "t2_echo_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.runreceipt_schema_hub_link_t3_crossref missing {key}")
...
if hook.get("t2_echo_sa") != "sa-0555":
    raise SystemExit("FAIL: t2_echo_sa must be sa-0555")
if "sa-0580-runreceipt-schema-hub-link-t3-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0580 attachment")

print("OK: validate-runreceipt-schema-hub-link-t3-crossref-v1 · canonical=sa-0505 · t2=sa-0555 · sa-0580")
PY
