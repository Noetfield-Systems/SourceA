...
bench = Path("archive/attachments/commercial/EXTERNAL_DESIGN_BENCHMARK_2026-06-15_v1.md")
for n in ("[agent-aispm-vendor].io", "notenic.ai", "fusegov.com", "witness.ai", "Overall", "Master scorecard"):
    if n not in bench.read_text(encoding="utf-8"):
        print(f"FAIL:  missing {n}")
        sys.exit(1)
print(f"PASS: {bench}")
