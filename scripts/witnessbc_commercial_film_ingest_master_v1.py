#!/usr/bin/env python3
"""Ingest founder Screen Studio WitnessBC master → site + Desktop.

Export from Screen Studio as:
  ~/Desktop/WitnessBC-Commercial-Master.mov
  ~/Desktop/WitnessBC-Commercial-Master.mp4
...
RECEIPT = SINA / "enforcement" / "witnessbc-commercial-film-receipt-v5.json"
OUT_SITE = ROOT / "witnessbc-site" / "assets" / "witnessbc-commercial.mp4"
OUT_DESKTOP = DESKTOP / "WitnessBC-Commercial.mp4"
QUALITY_BAR = ROOT / "data" / "video-quality-bar-v1.json"

CANDIDATES = [
    DESKTOP / "WitnessBC-Commercial-Master.mov",
    DESKTOP / "WitnessBC-Commercial-Master.mp4",
    DESKTOP / "WitnessBC-Commercial-Short-Master.mp4",
]
...
def find_master(explicit: Path | None = None) -> tuple[Path, str]:
    ...
    raise SystemExit(
        "FAIL: no WitnessBC commercial master on Desktop.\n"
        "  Record Proof Lab in Screen Studio (4K, slow cursor, hover-before-click).\n"
        "  Export as ~/Desktop/WitnessBC-Commercial-Master.mov\n"
        "  Quality bar: data/video-quality-bar-v1.json"
    )


def ingest(*, source: Path | None = None) -> dict:
    src, tier = find_master(source)
    work = OUT_SITE.parent / "witnessbc-commercial-ingest.mp4"
    ...
    receipt = {
        "schema": "witnessbc-commercial-film-receipt-v5",
        "at": _now(),
        "status": "embed_live",
        "title": "WitnessBC — Proof Lab",
        "product": "witnessbc",
        "grade": grade,
        "source_tier": tier,
        "source_path": str(src),
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Ingest WitnessBC Screen Studio master")
    ap.add_argument("--source", type=Path, default=None)
    ap.add_argument("--json", action="store_true")
    args = ap.parse_args()
    try:
        receipt = ingest(source=args.source)
    ...
    if args.json:
        ...
        print(
            f"WitnessBC commercial ingested · grade {receipt.get('grade')} · "
