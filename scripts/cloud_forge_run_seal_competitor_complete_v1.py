#!/usr/bin/env python3
"""Seal competitor-1000 Cloud Forge Run complete — pointer · pipeline · Mac phase · factory_now."""
...
RECEIPT = SINA / "cloud-forge-run-competitor-complete-receipt-v1.json"
...
def seal(*, cloud_row: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    batch_id = int(obs.get("batch_id") or 10)
    ...
    new_ptr = {
        ...
        **archives,
        "drain_status": "competitor_1000_complete",
        "registry_exhausted": True,
        "queue_batch_complete": True,
        "completed_at": _now(),
        "cloud_sec_range": "CLOUD-SEC-001..CLOUD-SEC-910",
        "maps_registry": "sa-mkt-0001..sa-mkt-1000",
    ...
    phase.update(
        {
            ...
            "next_motor_ssot": "data/forge-real-blueprints-v01.json",
            "rebuilt_at": _now(),
            "rebuilt_by": "cloud_forge_run_seal_competitor_complete_v1",
        }
    )
    ...
    row = {
        "ok": True,
        "schema": "cloud-forge-run-competitor-complete-receipt-v1",
        "at": _now(),
        "head": head,
        "last_completed": last,
        "batch_id": batch_id,
        ...
        "for_founder": {
            "show_this": (
                f"Competitor-1000 Cloud Forge Run SEALED · {last} · batch {batch_id} complete · "
                "next motor: forge-real-blueprints-v01"
            ),
        },
    }
    _write(RECEIPT, row)
