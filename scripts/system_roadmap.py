...
GOVERNANCE_ENTRY_DOC = "SINA_GOVERNANCE_ENTRY_LOCKED_v1.md"
AUTHORITY_INDEX_DOC = "SINA_AUTHORITY_INDEX_MAP_LOCKED_v1.md"
...
def _build_world_target_map() -> dict[str, Any]:
    ...
    if _phase_d_complete():
        if _gate_is_enforce():
            out["core_gap"] = (
                ...
                "ENFORCE gate live — remaining: L0/L1 live signals + embeddings + outcome ."
            )
        else:
            out["core_gap"] = (
                f"Pre-LLM world model complete (D1–D16, {shipped}/16 SSOT artifacts). "
    ...
    if _phase_d_complete():
        ...
        if not _artifact_exists("eval_packet_v1_report.json"):
            not_here.append("Eval-1 outcome  (structural packet vs raw)")
        else:
            try:
