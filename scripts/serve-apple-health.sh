#!/usr/bin/env bash
# Apple Health — standalone mini app (:13025). Does NOT start Sina Command hub.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${APPLE_HEALTH_PORT:-13025}"
PIDFILE="${HOME}/.sina/apple-health-server.pid"
LOGFILE="${HOME}/.sina/apple-health-server.log"
...
export APPLE_HEALTH_PORT="$PORT"
export APPLE_HEALTH_STANDALONE=1
...
if health_ok; then
  echo "Apple Health already running → http://127.0.0.1:${PORT}/"
  exit 0
fi
...
echo "Starting Apple Health → http://127.0.0.1:${PORT}/"
: >"$LOGFILE"
nohup python3 "$ROOT/scripts/apple-health-server.py" >>"$LOGFILE" 2>&1 &
echo $! >"$PIDFILE"
