#!/usr/bin/env bash
# validate-two-clock-synthesis-lessons-t2-crossref-v1.sh — sa-0574 ACT T2 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0574-two-clock-synthesis-lessons-t2-crossref_LOCKED_v1.md"
...
assert cross.is_file(), "missing sa-0574 cross-ref doc"
...
hook = (pp.get("signals_auto") or {}).get("two_clock_synthesis_lessons_t2_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "t1_echo_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.two_clock_synthesis_lessons_t2_crossref missing {key}")
...
if "sa-0574-two-clock-synthesis-lessons-t2-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0574 attachment")

print("OK: validate-two-clock-synthesis-lessons-t2-crossref-v1 · canonical=sa-0524 · t1=sa-0549 · sa-0574")
PY
