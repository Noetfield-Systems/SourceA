#!/usr/bin/env bash
# validate-product-factory-roadmap-hub-signals-t3-crossref-v1.sh — sa-0592 ACT T3 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0592-product-factory-roadmap-hub-signals-t3-crossref_LOCKED_v1.md"
...
assert cross.is_file(), "missing sa-0592 cross-ref doc"
...
hook = (pp.get("signals_auto") or {}).get("product_factory_roadmap_hub_signals_t3_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "t1_echo_sa", "t2_echo_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.product_factory_roadmap_hub_signals_t3_crossref missing {key}")
if hook.get("canonical_sa") != "sa-0517":
    raise SystemExit("FAIL: canonical_sa must be sa-0517")
if hook.get("t1_echo_sa") != "sa-0542":
    raise SystemExit("FAIL: t1_echo_sa must be sa-0542")
if hook.get("t2_echo_sa") != "sa-0567":
    raise SystemExit("FAIL: t2_echo_sa must be sa-0567")
if "sa-0592-product-factory-roadmap-hub-signals-t3-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0592 attachment")

print("OK: validate-product-factory-roadmap-hub-signals-t3-crossref-v1 · canonical=sa-0517 · t2=sa-0567 · sa-0592")
PY
