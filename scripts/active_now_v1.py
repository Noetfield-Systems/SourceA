...
def load_active_now() -> dict:
    ...
    file_fresh: dict = {"ok": True, "fresh": True, "stale": False}
    try:
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        from k1_read_gate_v1 import active_now_file_fresh  # noqa: WPS433

        file_fresh = active_now_file_fresh()
    except Exception:
        pass
    return {
        "ok": True,
        ...
        "fresh": bool(file_fresh.get("fresh", True)),
        "stale": bool(file_fresh.get("stale", False)),
        "file_age_hours": file_fresh.get("age_hours"),
    }
