...
bash "$ROOT/scripts/validate-portfolio--pick-v1.sh" >/dev/null 2>&1 \
  && pass " pick scripts" \
  || fail_line " pick validate"

python3 -c "import json; assert json.load(open('$ROOT/data/portfolio--1000-manifest-v1.json')).get('ok')" \
  && pass " manifest ok" \
  || fail_line " manifest"
...
for stack in sourcea witnessbc noetfield trustfield virlux; do
  if python3 "$ROOT/scripts/pick-portfolio--plan.py" --stack "$stack" --limit 1 --json >/dev/null 2>&1; then
...
if python3 "$ROOT/scripts/portfolio__forge_dispatch_v1.py" --stack sourcea --dry-run --json >/dev/null 2>&1; then
