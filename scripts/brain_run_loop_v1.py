...
def run_loop(*, turns: int = 10, founder_phrase: str = "run the loop") -> dict:
    ...
    # 0 SELF-HEAL (mandatory Brain session start)
    sys.path.insert(0, str(SCRIPTS))
    from brain_self_heal_startup_v1 import run_self_heal_startup  # noqa: WPS433

    heal = run_self_heal_startup(caller="brain_run_loop_v1")
    add(
        "self_heal_startup",
        "Mandatory hub · validators · queue · preflight before ASF surface",
        {
            "ok": heal.get("ok"),
            "founder_ready": heal.get("founder_ready"),
            "founder_message": heal.get("founder_message"),
            "p0_blockers": heal.get("p0_blockers"),
        },
    )
    if not heal.get("ok"):
        trace = {
            ...
            "steps": steps,
            "next_brain_action": "fix P0 blockers — do not surface ready to ASF",
        }
        _write(trace)
        return trace
...
def main() -> int:
    ...
    if freeze.is_file():
        print(json.dumps({"ok": False, "reason": "AUTORUN_DISABLED", "flag": str(freeze)}, indent=2))
        return 0
