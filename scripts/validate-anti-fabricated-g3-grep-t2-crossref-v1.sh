#!/usr/bin/env bash
# validate-anti-fabricated-g3-grep-t2-crossref-v1.sh — sa-0561 ACT T2 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0561-anti-fabricated-g3-grep-t2-crossref_LOCKED_v1.md"
...
assert cross.is_file(), "missing sa-0561 cross-ref doc"
...
hook = (pp.get("signals_auto") or {}).get("anti_fabricated_g3_grep_t2_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "t1_echo_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.anti_fabricated_g3_grep_t2_crossref missing {key}")
...
if "sa-0561-anti-fabricated-g3-grep-t2-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0561 attachment")

print("OK: validate-anti-fabricated-g3-grep-t2-crossref-v1 · canonical=sa-0511 · t1=sa-0536 · sa-0561")
PY
