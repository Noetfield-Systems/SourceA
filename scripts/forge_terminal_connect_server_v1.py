    ...
    def do_POST(self):
        ...
        if path == "/api/zcp/v1":
            ...
            try:
                ...
                body = {}
            act = str(body.get("action") or "ingest").strip().lower()
            if act in ("parse", "zcp_parse"):
                ...
                result = critic_gate(output if isinstance(output, dict) else {})
            elif act == "receipts":
                from zcp.zcp_ingest_v1 import list_receipts  # noqa: WPS433

                result = {"ok": True, "receipts": list_receipts(int(body.get("limit") or 20))}
            else:
...
def main() -> None:
    ...
    print("ZCP: open http://127.0.0.1:{}/terminal/ — paste FIX:/TASK/CRITIC in chat (not this shell)".format(PORT))
    print("ZCP CLI: python3 scripts/zcp_bridge_v1.py ingest -i 'FIX:\\ntarget: path\\nissue: …'")
    try:
