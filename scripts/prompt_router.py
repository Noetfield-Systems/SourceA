...
def _pick_competitor_plan(stack: str) -> dict[str, Any] | None:
    pick_script = SCRIPTS / "pick-portfolio-competitor-plan.py"
...
def _pick_for_lane(lane: str, *, pack: str = "") -> dict[str, Any] | None:
    if pack == "competitor":
        stack = COMPETITOR_LANE_STACK.get(lane)
        if stack:
            return _pick_competitor_plan(stack)
        return None
    if lane == "sourcea":
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="competitor — portfolio competitor-1000 pack (FORGE cloud)",
    )
    ...
    elif lane in COMPETITOR_LANE_STACK and pack == "competitor":
        pick = _pick_for_lane(lane, pack="competitor")
