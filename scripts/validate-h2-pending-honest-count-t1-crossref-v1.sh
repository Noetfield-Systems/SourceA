#!/usr/bin/env bash
# validate-h2-pending-honest-count-t1-crossref-v1.sh — sa-0843 T1 dedup cross-ref → canonical sa-0818
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT/scripts"

fail() { echo "FAIL: validate-h2-pending-honest-count-t1-crossref-v1 — $*" >&2; exit 1; }

CROSS="$ROOT/archive/attachments/2026-06-15/sa-0843-h2-pending-honest-count-t1-crossref_LOCKED_v1.md"
CANONICAL="$ROOT/scripts/validate-h2-pending-honest-count-v1.sh"
RECEIPT="$ROOT/receipts/sa-0818-receipt.json"

[[ -f "$CROSS" ]] || fail "missing sa-0843 cross-ref doc"
...
cross = Path("../archive/attachments/2026-06-15/sa-0843-h2-pending-honest-count-t1-crossref_LOCKED_v1.md")
...
print("OK: sa-0843 cross-ref doc cites sa-0818 canonical")
PY

bash "$CANONICAL" >/dev/null || fail "canonical validate-h2-pending-honest-count-v1"

echo "OK: validate-h2-pending-honest-count-t1-crossref-v1 · canonical=sa-0818 · sa-0843"
