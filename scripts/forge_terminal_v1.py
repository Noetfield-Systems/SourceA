...
SITE_INTELLIGENCE_EVENTS = SINA / "site-intelligence-events-v1.jsonl"
SITE_INTELLIGENCE_SITES = ("sourcea.app", "noetfield.com", "trustfield.ca")
...
def critic_observe_payload(body: dict[str, Any]) -> dict[str, Any]:
    """Lightweight Critic Observer: fast checks, no blocking model dependency."""
    text = str(body.get("text") or body.get("founder_text") or "").strip()
    answer = str(body.get("assistant_response") or body.get("response") or "").strip()
    quality = body.get("quality_gate") if isinstance(body.get("quality_gate"), dict) else {}
    run_id = str(body.get("run_id") or "").strip()
    living_chat = bool(body.get("living_chat"))
    combined = f"{text}\n{answer}".lower()

    concerns: list[str] = []
    if not answer:
        concerns.append("Builder produced no visible answer.")
    if len(answer) > 1800:
        concerns.append("Builder answer is long; founder may need a shorter bottom line.")
    if "..." in answer[:120] or "thinking" in answer[:120].lower():
        concerns.append("Builder may still be in a pending state.")
    if re.search(r"\b(delete|drop table|force push|secret|password|private key|deploy)\b", combined):
        concerns.append("High-impact or secret-sensitive wording needs explicit proof before action.")
    if re.match(r"^\s*(fix|task|critic)\s*:", text, re.I):
        concerns.append("Structured block detected; route and receipt should be visible.")
    if quality and quality.get("execution_allowed") is False and not living_chat:
        concerns.append(f"Quality gate blocks execution: {quality.get('verdict') or 'review required'}.")
    if quality and quality.get("score") is not None and not living_chat:
        try:
            if float(quality.get("score")) < 80:
                concerns.append("Quality score is below enterprise threshold.")
        except (TypeError, ValueError):
            pass

    verdict = "PASS" if not concerns else "CONCERN"
    if not answer:
        verdict = "CRITIC"
    what_i_see = (
        "Builder replied and the critic has enough context to review it."
        if answer
        else "I do not see a completed Builder reply yet."
    )
    if text:
        what_i_see = f"Founder asked: {text[:160]}"
    if concerns:
        risk = concerns[0]
    elif living_chat:
        risk = "No chat risk detected. Execution still needs a TASK/FIX and proof before acting."
    else:
        risk = "No blocking risk detected. Keep the next step explicit and receipt-backed."
    next_check = (
        "Ask Builder for proof, exact file path, or a narrower TASK/FIX block before execution."
        if concerns
        else "Proceed with the next bounded step; keep proof visible in the receipt/run card."
    )

    return {
        "ok": True,
        "schema": "forge-critic-observer-v1",
        "at": _now(),
        "run_id": run_id,
        "verdict": verdict,
        "state": "pass" if verdict == "PASS" else "concern",
        "what_i_see": what_i_see,
        "risk": risk,
        "next_check": next_check,
        "concerns": concerns,
        "advice_for_builder": next_check,
    }


def _site_intel_site(raw: str) -> str:
    value = (raw or "").strip().lower().replace("https://", "").replace("http://", "").strip("/")
    if value.startswith("www."):
        value = value[4:]
    if value in SITE_INTELLIGENCE_SITES:
        return value
    if "noet" in value:
        return "noetfield.com"
    if "trust" in value:
        return "trustfield.ca"
    return "sourcea.app"


def _site_intel_classify(text: str, kind: str) -> dict[str, str]:
    blob = (text or "").lower()
    category = "feedback"
    if kind in ("probe", "health", "telemetry") or re.search(r"\b(500|timeout|failed write|db|database|receipt|api|railway|supabase|health|probe)\b", blob):
        category = "silent_failure"
    elif re.search(r"\b(prompt injection|ignore previous|system prompt|secret|password|token|private key|xss|csrf|auth)\b", blob):
        category = "security"
    elif re.search(r"\b(confusing|unclear|can't find|hard to use|small|slow|stuck|not responding|broken ui)\b", blob):
        category = "ux"
    elif re.search(r"\b(price|buy|demo|lead|conversion|signup|checkout|book)\b", blob):
        category = "conversion"

    risk = "low"
    if category in ("security", "silent_failure"):
        risk = "high"
    elif category in ("ux", "conversion") or re.search(r"\b(stuck|broken|conflict|leak|failure)\b", blob):
        risk = "medium"

    next_action = "MONITOR"
    if category == "security":
        next_action = "CRITIC"
    elif category == "silent_failure":
        next_action = "FIX"
    elif category in ("ux", "conversion"):
        next_action = "TASK"

    return {"category": category, "risk": risk, "next_action": next_action}


def _site_intel_title(text: str) -> str:
...
def site_intelligence_ingest(body: dict[str, Any]) -> dict[str, Any]:
    text = str(body.get("text") or body.get("message") or body.get("feedback") or "").strip()
    if not text:
        return {"ok": False, "error": "empty_signal", "message": "Site intelligence signal needs text."}
    kind = str(body.get("kind") or body.get("source") or "feedback").strip().lower()
    site = _site_intel_site(str(body.get("site") or body.get("domain") or "sourcea.app"))
    classified = _site_intel_classify(text, kind)
    event_id = "sih-" + uuid.uuid4().hex[:12]
    row = {
        "schema": "site-intelligence-event-v1",
        "id": event_id,
        "at": _now(),
        "site": site,
        "kind": kind,
        "title": _site_intel_title(text),
        "text": text[:4000],
    ...
    _append_jsonl(SITE_INTELLIGENCE_EVENTS, row)
    return {"ok": True, "event": row, "path": str(SITE_INTELLIGENCE_EVENTS)}


def _site_intel_events(limit: int = 500) -> list[dict[str, Any]]:
    if not SITE_INTELLIGENCE_EVENTS.is_file():
        return []
    rows: list[dict[str, Any]] = []
    try:
        for line in SITE_INTELLIGENCE_EVENTS.read_text(encoding="utf-8").splitlines()[-limit:]:
            if not line.strip():
                continue
            try:
...
def site_intelligence_summary(body: dict[str, Any] | None = None) -> dict[str, Any]:
    rows = _site_intel_events()
    site_filter = _site_intel_site(str((body or {}).get("site") or "")) if (body or {}).get("site") else ""
    if site_filter:
        rows = [r for r in rows if r.get("site") == site_filter]

    by_site: dict[str, dict[str, Any]] = {
        site: {"site": site, "total": 0, "high": 0, "medium": 0, "low": 0, "categories": {}, "next_actions": {}}
        for site in SITE_INTELLIGENCE_SITES
    }
    repeated: dict[str, dict[str, Any]] = {}
    for row in rows:
        site = str(row.get("site") or "sourcea.app")
        bucket = by_site.setdefault(site, {"site": site, "total": 0, "high": 0, "medium": 0, "low": 0, "categories": {}, "next_actions": {}})
        risk = str(row.get("risk") or "low")
        category = str(row.get("category") or "feedback")
        action = str(row.get("next_action") or "MONITOR")
        bucket["total"] += 1
        bucket[risk] = int(bucket.get(risk) or 0) + 1
        bucket["categories"][category] = int(bucket["categories"].get(category) or 0) + 1
        bucket["next_actions"][action] = int(bucket["next_actions"].get(action) or 0) + 1
        key = f"{site}:{category}:{str(row.get('title') or '')[:48].lower()}"
        repeated.setdefault(key, {"site": site, "category": category, "title": row.get("title"), "count": 0, "risk": risk})
        repeated[key]["count"] += 1
        if risk == "high":
            repeated[key]["risk"] = "high"

    recent = list(reversed(rows[-20:]))
    repeated_rows = sorted(
        [v for v in repeated.values() if int(v.get("count") or 0) > 1],
        key=lambda x: (str(x.get("risk") != "high"), -int(x.get("count") or 0)),
    )[:12]
    total = len(rows)
    return {
        "ok": True,
        "schema": "site-intelligence-summary-v1",
        "at": _now(),
        "path": str(SITE_INTELLIGENCE_EVENTS),
        "total": total,
        "sites": list(by_site.values()),
        "recent": recent,
        "repeated": repeated_rows,
        "next_actions": {
            "FIX": sum(int(s["next_actions"].get("FIX") or 0) for s in by_site.values()),
            "TASK": sum(int(s["next_actions"].get("TASK") or 0) for s in by_site.values()),
            "CRITIC": sum(int(s["next_actions"].get("CRITIC") or 0) for s in by_site.values()),
            "MONITOR": sum(int(s["next_actions"].get("MONITOR") or 0) for s in by_site.values()),
        },
    }
...
def run_terminal(
    *,
    ...
    ws_root = None if living_chat else _resolve_workspace_root(workspace_path=workspace_path, workspace_id=workspace_id)
    chat_only = living_chat or not ws_root
    if not chat_only:
    ...
    if fast:
        forge = {
            "ok": True,
            "mode": "live_chat" if chat_only else "fast_run",
            "used_llm": False,
            "receipt_path": None,
            "workspace": ws_label,
            "chat_only": chat_only,
    ...
    living_fast = bool(fast and chat_only)

    if full_llm:
        if chat_only:
            if living_fast:
                scoped_user = (
                    "CONTEXT: Chat Unify living chat — no workspace folder open.\n"
                    "Reply directly and concisely. Match the founder's length: short question → short answer.\n"
                    "No mandatory headings unless they asked for a report. Plain English.\n\n"
                    f"Message:\n{mission[:10000]}"
                )
            else:
                scoped_user = (
    ...
    raw_trim = (raw_response or "").strip()
    if living_fast:
        simplified = raw_trim[:2500]
        simplified_row = {"founder_text": simplified, "method": "living_direct"}
    elif len(raw_trim) < 500 and not _looks_like_json_blob(raw_trim):
        simplified = raw_trim
        simplified_row = {"founder_text": simplified, "method": "direct"}
    else:
    ...
    quality = evaluate_quality_gate(
        ...
        full_llm=full_llm,
        eval_shadow=bool(full_llm) and not living_fast,
    )
...
def handle_post(body: dict[str, Any]) -> dict[str, Any]:
    ...
    if action == "status_light":
        return status_light(workspace_path=ws_path)
    if action == "critic_observe":
        return critic_observe_payload(body)
    if action == "site_intelligence_ingest":
        return site_intelligence_ingest(body)
    if action == "site_intelligence_summary":
        return site_intelligence_summary(body)
    ...
    if action == "chat_turn":
        ...
        row = run_terminal(
            ...
            provider=str(body.get("provider") or "auto"),
            model=str(body.get("model") or ""),
            workspace_id=ws_id if not body.get("living_chat") else None,
            workspace_path=ws_path if not body.get("living_chat") else None,
            living_chat=bool(body.get("living_chat")),
        )
        if row.get("ok"):
            from forge_terminal_desktop_mesh_v1 import append_turn  # noqa: WPS433
    ...
    if action == "open_user_workspace":
        ...
        return opened
    if action in ("zcp_parse", "zcp"):
        from zcp.zcp_ingest_v1 import parse_only  # noqa: WPS433

        return parse_only(
            str(body.get("input") or body.get("text") or ""),
            complexity=str(body.get("complexity") or "medium"),
        )
    if action == "zcp_ingest":
        from zcp.zcp_ingest_v1 import ingest  # noqa: WPS433

        plane = str(body.get("plane") or "auto")
        if plane not in ("parse_only", "cursor", "cloud", "auto"):
            plane = "auto"
        return ingest(
            input_text=str(body.get("input") or body.get("text") or ""),
            complexity=str(body.get("complexity") or "medium"),
            project_id=str(body.get("project_id") or "zcp"),
            dispatch=bool(body.get("dispatch")),
            plane=plane,  # type: ignore[arg-type]
            dry_run=bool(body.get("dry_run")),
        )
    if action == "zcp_critic":
        from zcp.zcp_ingest_v1 import critic_gate  # noqa: WPS433
        ...
        if isinstance(output, str):
            try:
                ...
                output = {"risk": ["invalid critic json"], "fix_priority": [], "drift_score": 10}
        if not isinstance(output, dict):
            output = {}
        return critic_gate(output)
    if action == "zcp_receipts":
        from zcp.zcp_ingest_v1 import list_receipts  # noqa: WPS433

        return {"ok": True, "receipts": list_receipts(int(body.get("limit") or 20))}
    return {
        "ok": False,
