#!/usr/bin/env python3
"""Founder pivot router — complex founder asks → route_pack logged (no repeat briefings).
...
def route_founder_pivot(
    ...
    *,
    ...
    row = {
        ...
        "law": "Complex founder pivot logged — agents read receipt; founder does not repeat",
    }
    if write:
        SINA.mkdir(parents=True, exist_ok=True)
        RECEIPT.write_text(json.dumps(row, indent=2) + "\n", encoding="utf-8")
