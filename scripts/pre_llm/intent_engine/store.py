"""D4 SSOT — ~/.sina/intent_engine_v1.json"""
...
INTENT_SSOT_PATH = STATE_DIR / "intent_engine_v1.json"
SCHEMA = "intent-engine-v1"


def load_snapshot() -> dict:
    if not INTENT_SSOT_PATH.is_file():
        return {}
    try:
        data = json.loads(INTENT_SSOT_PATH.read_text(encoding="utf-8"))
...
def write_canonical(*, canonical: dict, task_id: str = "default") -> None:
    ...
    envelope = load_snapshot() if INTENT_SSOT_PATH.is_file() else {"schema": SCHEMA, "builds": {}}
    envelope["schema"] = SCHEMA
    envelope["generated_at"] = canonical.get("generated_at")
    envelope["path"] = str(INTENT_SSOT_PATH)
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as fh:
            ...
            fh.write("\n")
        os.replace(tmp, INTENT_SSOT_PATH)
    except Exception:
        try:
            os.unlink(tmp)
        except OSError:
