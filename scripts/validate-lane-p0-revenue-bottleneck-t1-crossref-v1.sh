#!/usr/bin/env bash
# validate-lane-p0-revenue-bottleneck-t1-crossref-v1.sh — sa-0535 ACT T1 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
export PATH="/opt/homebrew/bin:/usr/local/bin:${PATH:-/usr/bin:/bin}"

python3 - <<'PY'
import json
from pathlib import Path

root = Path(__file__).resolve().parents[1]
cross = root / "archive/attachments/2026-06-14/sa-0535-lane-p0-revenue-bottleneck-t1-crossref_LOCKED_v1.md"
canonical_doc = root / "archive/attachments/2026-06-14/sa-0510-lane-p0-revenue-bottleneck_LOCKED_v1.md"
hub_script = root / "scripts/strategic_synthesis_hub.py"
receipt = root / "receipts/sa-0510-receipt.json"

assert cross.is_file(), "missing sa-0535 cross-ref doc"
text = cross.read_text(encoding="utf-8")
for needle in (
    "sa-0510",
    "sa-0535",
    "sa-0510-lane-p0-revenue-bottleneck_LOCKED_v1.md",
    "strategic_synthesis_hub.py",
):
    assert needle in text, f"cross-ref missing {needle}"
assert canonical_doc.is_file(), "canonical sa-0510 attachment missing"
assert hub_script.is_file(), "strategic_synthesis_hub.py missing"
assert receipt.is_file(), "canonical sa-0510 receipt missing"
for marker in ("build-sina-command-panel.py", "hub_self_refresh"):
    assert marker not in text, f"T1 cross-ref must not duplicate implementation prose ({marker})"

pp = json.loads((root / "PROGRAM_PROGRESS.json").read_text(encoding="utf-8"))
hook = (pp.get("signals_auto") or {}).get("lane_p0_revenue_bottleneck_t1_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.lane_p0_revenue_bottleneck_t1_crossref missing {key}")
if hook.get("canonical_sa") != "sa-0510":
    raise SystemExit("FAIL: canonical_sa must be sa-0510")
if "sa-0535-lane-p0-revenue-bottleneck-t1-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0535 attachment")

print("OK: validate-lane-p0-revenue-bottleneck-t1-crossref-v1 · canonical=sa-0510 · sa-0535")
PY

bash validate-lane-p0-revenue-bottleneck-v1.sh
