#!/usr/bin/env bash
# validate-gtm-moat-governance-scoreboard-t3-crossref-v1.sh — sa-0595 ACT T3 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0595-gtm-moat-governance-scoreboard-t3-crossref_LOCKED_v1.md"
...
assert cross.is_file(), "missing sa-0595 cross-ref doc"
...
hook = (pp.get("signals_auto") or {}).get("gtm_moat_governance_scoreboard_t3_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "t1_echo_sa", "t2_echo_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.gtm_moat_governance_scoreboard_t3_crossref missing {key}")
...
if hook.get("t2_echo_sa") != "sa-0570":
    raise SystemExit("FAIL: t2_echo_sa must be sa-0570")
if "sa-0595-gtm-moat-governance-scoreboard-t3-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0595 attachment")

print("OK: validate-gtm-moat-governance-scoreboard-t3-crossref-v1 · canonical=sa-0520 · t2=sa-0570 · sa-0595")
PY
