#!/usr/bin/env bash
# validate-commercial-lane-g3-vault-t1-crossref-v1.sh — sa-0550 ACT T1 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0550-commercial-lane-g3-vault-t1-crossref_LOCKED_v1.md"
...
assert cross.is_file(), "missing sa-0550 cross-ref doc"
...
for needle in (
    ...
    "validate-commercial-lane-g3-vault-evidence-v1.sh",
):
...
hook = (pp.get("signals_auto") or {}).get("commercial_lane_g3_vault_t1_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.commercial_lane_g3_vault_t1_crossref missing {key}")
if hook.get("canonical_sa") != "sa-0525":
    raise SystemExit("FAIL: canonical_sa must be sa-0525")
if "sa-0550-commercial-lane-g3-vault-t1-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0550 attachment")

print("OK: validate-commercial-lane-g3-vault-t1-crossref-v1 · canonical=sa-0525 · sa-0550")
PY

bash validate-commercial-lane-g3-vault-evidence-v1.sh
