#!/usr/bin/env bash
# validate-anti-fabricated-g3-grep-t1-crossref-v1.sh — sa-0536 ACT T1 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0536-anti-fabricated-g3-grep-t1-crossref_LOCKED_v1.md"
canonical_doc = root / "archive/attachments/2026-06-14/sa-0511-anti-fabricated-g3_LOCKED_v1.md"
canonical_validator = root / "scripts/validate-anti-fabricated-g3-grep-v1.sh"
receipt = root / "receipts/sa-0511-receipt.json"

assert cross.is_file(), "missing sa-0536 cross-ref doc"
...
assert canonical_validator.is_file(), "canonical validator missing"
...
hook = (pp.get("signals_auto") or {}).get("anti_fabricated_g3_grep_t1_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.anti_fabricated_g3_grep_t1_crossref missing {key}")
if hook.get("canonical_sa") != "sa-0511":
    raise SystemExit("FAIL: canonical_sa must be sa-0511")
if "sa-0536-anti-fabricated-g3-grep-t1-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0536 attachment")

print("OK: validate-anti-fabricated-g3-grep-t1-crossref-v1 · canonical=sa-0511 · sa-0536")
PY
