#!/usr/bin/env bash
# Wire n8n + cloud drain auto-runtime — start n8n, import/activate wf-cloud-drain-auto-v1.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SINA="${HOME}/.sina"
RECEIPT="${SINA}/n8n-cloud-drain-wire-receipt-v1.json"
...
echo "→ Regenerating + importing wf-cloud-drain-auto-v1…"
...
WF = ROOT / "n8n/workflows/wf-cloud-drain-auto-v1.json"
N8N_DB = Path.home() / ".n8n/database.sqlite"
data = json.loads(WF.read_text())
wf_id = data.get("id") or str(uuid.uuid4())
data["id"] = wf_id
data["active"] = True
tmp = Path("/tmp/n8n-import-wf-cloud-drain-auto-v1.json")
...
if N8N_DB.is_file():
    ...
    con.execute("UPDATE workflow_entity SET active = 1 WHERE name = ?", ("wf-cloud-drain-auto-v1",))
    con.commit()
    row = con.execute("SELECT id,name,active FROM workflow_entity WHERE name=? ORDER BY updatedAt DESC LIMIT 1", ("wf-cloud-drain-auto-v1",)).fetchone()
    con.close()
    print("workflow:", row)
PY

echo "→ CF 24/7 cron (primary scheduler when Mac sleeps)…"
CF_URL="https://sourcea-cloud-drain-tick-v1.witness-bc.workers.dev"
...
from pathlib import Path
r = {
  'schema': 'n8n-cloud-drain-wire-receipt-v1',
  'at': datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ'),
  'n8n_running': True,
  'n8n_url': 'http://127.0.0.1:5678',
  'workflow': 'wf-cloud-drain-auto-v1',
  'workflow_cron': '*/15 * * * *',
  'cf_worker_url': 'https://sourcea-cloud-drain-tick-v1.witness-bc.workers.dev',
  'cf_health': json.loads('''$CF_HEALTH''' if '''$CF_HEALTH'''.strip() else '{}'),
  'primary_scheduler': 'cloudflare_cron',
  'backup_scheduler': 'n8n_mac_awake',
  'for_founder': {'show_this': 'n8n UP · cloud drain workflow active · CF cron is 24/7 primary'},
}
Path(os.path.expanduser('~/.sina/n8n-cloud-drain-wire-receipt-v1.json')).write_text(json.dumps(r, indent=2)+'\n')
