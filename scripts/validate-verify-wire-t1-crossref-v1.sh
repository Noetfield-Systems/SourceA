#!/usr/bin/env bash
# validate-verify-wire-t1-crossref-v1.sh — sa-0527 ACT T1 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0527-verify-wire-t1-crossref_LOCKED_v1.md"
...
assert cross.is_file(), "missing sa-0527 cross-ref doc"
text = cross.read_text(encoding="utf-8")
for needle in ("sa-0502", "sa-0522", "sa-0527", "sa-0502-verify-wire-runreceipt-schema_LOCKED_v1.md"):
...
for marker in ("| Check | Result |", "run.jsonl · summary.json · receipt.html · evidence.snapshot.json"):
    assert marker not in text, f"T1 cross-ref must not duplicate schema table ({marker})"

pp = json.loads((root / "PROGRAM_PROGRESS.json").read_text(encoding="utf-8"))
hook = (pp.get("signals_auto") or {}).get("verify_wire_t1_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "hardening_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.verify_wire_t1_crossref missing {key}")
...
if "sa-0527-verify-wire-t1-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0527 attachment")

print("OK: validate-verify-wire-t1-crossref-v1 · canonical=sa-0502 · hardening=sa-0522 · sa-0527")
PY
