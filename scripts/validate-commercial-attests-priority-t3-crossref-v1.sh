#!/usr/bin/env bash
# validate-commercial-attests-priority-t3-crossref-v1.sh — sa-0596 ACT T3 dedup cross-ref
set -euo pipefail
cd "$(dirname "$0")"
...
cross = root / "archive/attachments/2026-06-14/sa-0596-commercial-attests-priority-t3-crossref_LOCKED_v1.md"
...
assert cross.is_file(), "missing sa-0596 cross-ref doc"
...
hook = (pp.get("signals_auto") or {}).get("commercial_attests_checklist_t3_crossref") or {}
for key in ("crossref_doc", "canonical_sa", "t1_echo_sa", "t2_echo_sa", "validator"):
    if key not in hook:
        raise SystemExit(f"FAIL: signals_auto.commercial_attests_checklist_t3_crossref missing {key}")
...
if hook.get("t2_echo_sa") != "sa-0571":
    raise SystemExit("FAIL: t2_echo_sa must be sa-0571")
if "sa-0596-commercial-attests-priority-t3-crossref" not in str(hook.get("crossref_doc", "")):
    raise SystemExit("FAIL: crossref_doc must point to sa-0596 attachment")

print("OK: validate-commercial-attests-priority-t3-crossref-v1 · canonical=sa-0521 · t2=sa-0571 · sa-0596")
PY
