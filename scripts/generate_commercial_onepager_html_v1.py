...
EXTERNAL_DESIGN_REF_HTML = SINA / "sourcea-[agent-aispm-vendor]-platform-onepager-v1.html"
EXTERNAL_DESIGN_REF_ARCHIVE = SOURCEA / "archive/attachments/commercial/sourcea-[agent-aispm-vendor]-platform-onepager-v1.html"
...
def bundle_[agent-aispm-vendor]_layout_html() -> str:
    """Self-contained SourceA landing — [agent-aispm-vendor].ai layout (fleet panel · pillars · matrix · 4-tier pricing)."""
    html = (LANDING_DIR / "[agent-aispm-vendor].html").read_text(encoding="utf-8")
...
def write_all() -> dict[str, str]:
    ...
    [agent-aispm-vendor] = bundle_[agent-aispm-vendor]_layout_html()
    EXTERNAL_DESIGN_REF_HTML.write_text([agent-aispm-vendor], encoding="utf-8")
    paths["[agent-aispm-vendor]"] = str(EXTERNAL_DESIGN_REF_HTML)
    EXTERNAL_DESIGN_REF_ARCHIVE.parent.mkdir(parents=True, exist_ok=True)
    EXTERNAL_DESIGN_REF_ARCHIVE.write_text([agent-aispm-vendor], encoding="utf-8")
    paths["[agent-aispm-vendor]_archive"] = str(EXTERNAL_DESIGN_REF_ARCHIVE)
    return paths
...
def main() -> int:
    ...
    parser.add_argument(
        "target",
        nargs="?",
        choices=["all", "noetfield", "ab1", "[agent-aispm-vendor]"],
        default="all",
    ...
    if args.target == "[agent-aispm-vendor]":
        selected = {"[agent-aispm-vendor]": paths["[agent-aispm-vendor]"], "[agent-aispm-vendor]_archive": paths["[agent-aispm-vendor]_archive"]}
