...
PLAN: healthy queue + INBOX on disk (orchestrator deliver)
...
TRACE: AUTO-TRACE-WORKER-AUTOLOOP-CAP-v1.0 · agent Auto
"""
...
LOCK_PATH = Path.home() / ".sina" / "goal1-auto-loop-lock-v1.json"
...
def _loop_running() -> dict | None:
    """Return live goal1_run_loop row if batch is in flight — do not STOP during prepare."""
    proc = subprocess.run(["pgrep", "-f", "goal1_run_loop_v1.py"], capture_output=True, text=True)
...
def prepare(*, turns: int = 10) -> dict:
    ...
    if not running and not autorun_flag:
        subprocess.run(
            [sys.executable, str(SCRIPTS / "stop_goal1_loop_v1.py"), "--note", "auto_loop_prepare"],
            cwd=str(ROOT),
            capture_output=True,
            text=True,
            timeout=30,
...
def spawn_detached(*, turns: int = 10, timeout_sec: int = 1800) -> dict:
    ...
    if not prep.get("ok"):
        return prep
    if prep.get("loop_already_running"):
        return {
            "ok": True,
            "status": "GOAL1_AUTO_LOOP_ALREADY_RUNNING",
    ...
    with LOG_PATH.open("a", encoding="utf-8") as fh:
        fh.write(
            f"\n[{_now()}] AUTO LOOP START turns={turns} queue={prep.get('queue')} "
        ...
        proc = subprocess.Popen(
            [
                sys.executable,
                str(SCRIPTS / "goal1_run_loop_v1.py"),
                str(turns),
                str(timeout_sec),
            ],
            cwd=str(ROOT),
    ...
    LOCK_PATH.write_text(
        json.dumps({"pid": proc.pid, "at": _now(), "turns": turns, "script": "goal1_run_loop_v1.py"}, indent=2) + "\n",
        encoding="utf-8",
    )
    return {
        "ok": True,
        "status": "GOAL1_AUTO_LOOP_STARTED",
        "pid": proc.pid,
        "turns": turns,
        "log_path": str(LOG_PATH),
        "queue": prep.get("queue"),
...
def main() -> int:
    p = argparse.ArgumentParser(description="Goal 1 deliver + detached auto loop")
