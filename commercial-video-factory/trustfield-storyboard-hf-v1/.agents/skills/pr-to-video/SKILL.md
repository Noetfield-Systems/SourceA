...
- verify-output **audio** exit 1 -> a voice wav / `bgm.wav` / `captions.html` exists logged but was NOT wired into `index.html` (the silent / caption-less render class). This is an upstream wiring bug — almost always empty `group_spec` voicePaths because prep ran without `--audio-meta`. **Do NOT render.** Re-run Step 5 prep with `--audio-meta ./audio_meta.json`, then re-run this Step 7(1) chain. `⚠`-prefixed lines (BGM / captions intended but never produced logged) are non-blocking generation gaps — render proceeds.
