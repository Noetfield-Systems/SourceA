...
function parseEnvFile(path: string): Record<string, string> {
  ...
  if (orCandidates.length > 0) {
    const valid = orCandidates.filter((candidate) =>
      candidate.startsWith("sk-or-v1-"),
    );
...
function collectVaultPaths(): string[] {
...
export function bootstrapForgeEnv(): void {
  if (bootstrapped) {
    return;
  }
...
export function getOpenRouterKey(): string {
  bootstrapForgeEnv();
  return (
    process.env.OPENROUTER_API_KEY_FORGE?.trim() ||
