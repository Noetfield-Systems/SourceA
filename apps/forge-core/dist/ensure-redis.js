...
function redisUrlFile() {
    return `${getStateRoot()}/redis.url`;
}
function redisLockFile() {
...
export async function pingRedis(url = getRedisUrl()) {
...
async function waitForSharedRedis(maxSeconds = 90) {
...
export async function ensureRedis() {
    ...
    if (process.env.FORGE_EMBEDDED_REDIS === "0") {
        ...
        throw new Error(`Redis unavailable at ${configured}. Start: node apps/forge-core/dist/ensure-redis-cli.js`);
    }
    try {
...
export async function stopEmbeddedRedis() {
    if (embeddedServer) {
