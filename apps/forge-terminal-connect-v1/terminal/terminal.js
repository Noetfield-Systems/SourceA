(function () {
  ...
  const UI_VERSION = "4.11.20-site-intelligence";
  ...
  const SIMPLE_IDE =
  !(window.parent !== window || qsBoot.get("embed") === "1") && qsBoot.get("full") !== "1";

  function isSimpleIde() {
    return SIMPLE_IDE || document.documentElement.classList.contains("forge-simple-ide");
  }

  function applyForgeDesktopShell() {
    document.documentElement.classList.add("forge-desktop-app");
    document.body.classList.add("forge-desktop-app");
    const app = $("forge-app");
    if (app) app.classList.add("forge-desktop-app");
    forceScrollUnlock();
    enableEditorWindow();
  ...
  async function initEngineRoute() {
    let service = "forge-terminal";
    try {
      const h = await fetch(window.location.origin + "/health", { cache: "no-store" }).then(function (r) {
        return r.json();
      });
      service = h.service || "forge-terminal";
    } catch (_) {
      /* offline bundle — treat desktop embed as Forge Terminal */
    }
    if (service === "chat-unify") {
    ...
    inChatUnifyShell = false;
    API = FORGE_API;
    document.documentElement.classList.remove("cu-engine-active", "forge-cu-combined");
    document.body.classList.remove("forge-cu-combined");
    if ($("forge-app")) {
      $("forge-app").classList.remove("cu-engine-active", "forge-cu-combined");
    }
    if (isDesktopApp() || qsBoot.get("desktop") === "1") {
      applyForgeDesktopShell();
    }
  }
  ...
  function applyChatUnifyCombinedLayout() {
    ...
    const machinesDeck = document.querySelector(".forge-machines-deck");
    if (machinesDeck) machinesDeck.setAttribute("hidden", "");
  ...
  async function cuEngineApi(body) {
    const res = await fetch(CU_ENGINE_API, {
      method: "POST",
    ...
    try {
      ...
      throw new Error(raw.slice(0, 200) || "Chat Unify engine failed");
    }
  }
  ...
  function isZcpMessage(text) {
    const t = (text || "").trim();
    if (!t) return false;
    return /^(FIX:|CRITIC:|\{[\s\S]*"task_id")/i.test(t);
  }
  ...
  function setSendEnabled(enabled) {
    ...
    if (composerBtn) composerBtn.disabled = !on;
  }

  function syncSendButtonState() {
    const text = ($("prompt-input") && $("prompt-input").value.trim()) || "";
    const mode = getForgeMode();
    let on = text.length > 0;
    if (opening && mode !== "chat") on = false;
    if (isZcpMessage(text)) {
      on = text.length > 0;
    } else if (mode === "chat") {
      if (chatSettings && chatSettings.require_workspace_for_chat) {
        on = on && !!workspacePath;
      }
    } else {
      on = on && !!workspacePath;
  ...
  async function ensureServerReachable() {
    ...
    try {
      ...
      if (e && e.name === "AbortError") {
        throw new Error(
          "Forge server not responding — start: python3 scripts/forge_terminal_connect_server_v1.py",
        );
      }
      throw new Error(
        "Forge server offline — start: python3 scripts/forge_terminal_connect_server_v1.py (port 13029)",
      );
    } finally {
      clearTimeout(timer);
    }
  ...
  async function api(body) {
    ...
    if (workspacePath && !payload.living_chat) payload.workspace_path = workspacePath;
    const t0 = performance.now();
    const headers = { "Content-Type": "application/json" };
    if (forgeToken) headers["X-Forge-Token"] = forgeToken;
    const ctrl = new AbortController();
    const abortMs = payload.action === "chat_turn" ? 65000 : 45000;
    ...
    try {
      ...
      if (e && e.name === "AbortError") {
        throw new Error("Request timed out after " + Math.round(abortMs / 1000) + "s — check server + API keys");
      }
      throw new Error(
        (e && e.message) ||
          "Cannot reach Forge server — run python3 scripts/forge_terminal_connect_server_v1.py",
      );
    } finally {
  ...
  function chatTurnTimeoutMs(liveLlm) {
    return liveLlm ? 45000 : 30000;
  }

  async function apiChatTurn(payload) {
    const ms = chatTurnTimeoutMs(!!(payload && payload.full_llm));
    let timer;
    const timeout = new Promise(function (_, reject) {
      timer = setTimeout(function () {
        reject(
          new Error(
            "Reply timed out after " +
              Math.round(ms / 1000) +
              "s — try again or turn off Live LLM for a faster draft reply",
          ),
        );
      }, ms);
    });
    try {
      return await Promise.race([api(payload), timeout]);
    } finally {
  ...
  function setLlmTether(state, detail) {
    const el = $("forge-llm-tether");
    if (!el) return;
    el.classList.remove("is-live", "is-thinking", "is-error", "is-offline");
    const label = detail || "";
    if (state === "thinking") {
      el.classList.add("is-thinking");
      el.textContent = "● LLM thinking…";
      el.title = label || "Waiting for model reply";
    } else if (state === "live") {
      el.classList.add("is-live");
      el.textContent = "● LLM live";
      el.title = label || "Model connected — keys loaded";
    } else if (state === "error") {
      el.classList.add("is-error");
      el.textContent = "● LLM error";
      el.title = label || "Reply failed — try again";
    } else {
      el.classList.add("is-offline");
      el.textContent = "● LLM offline";
      el.title = label || "Add API keys in ~/.sina/secrets.env";
    }
  }

  function clearTypingBubble() {
    if (activeTypingBubble && activeTypingBubble.parentNode) {
      activeTypingBubble.parentNode.removeChild(activeTypingBubble);
    }
    activeTypingBubble = null;
  }

  function setCriticState(state, label) {
    const el = $("critic-status");
    if (!el) return;
    el.classList.remove("is-idle", "is-thinking", "is-pass", "is-concern", "is-offline");
    el.classList.add(
      state === "thinking"
        ? "is-thinking"
        : state === "pass"
    ...
    el.textContent = label || (state === "thinking" ? "Checking" : state === "pass" ? "PASS" : "Watching");
  }

  function renderCritic(row) {
    const see = $("critic-see");
    const risk = $("critic-risk");
    const next = $("critic-next");
    const intro = $("critic-intro");
    if (!see || !risk || !next) return;
    if (!row || row.ok === false) {
      setCriticState("offline", "Offline");
      risk.textContent = (row && (row.message || row.error)) || "Critic check failed.";
      next.textContent = "Builder chat still works. Retry Critic Check when the server is healthy.";
      return;
    }
    const state = row.state || (row.verdict === "PASS" ? "pass" : "concern");
    setCriticState(state, row.verdict || (state === "pass" ? "PASS" : "CONCERN"));
    see.textContent = row.what_i_see || "Critic reviewed the Builder reply.";
    risk.textContent = row.risk || "No blocking risk detected.";
    next.textContent = row.next_check || row.advice_for_builder || "Keep the next step bounded.";
    if (intro) {
      intro.textContent = row.run_id
        ? "Watching run " + row.run_id + " without blocking Builder chat."
        : "Watching Builder output.";
    }
  }

  async function runCriticObserver(payload) {
    setCriticState("thinking", "Checking");
    try {
      const row = await api(Object.assign({ action: "critic_observe" }, payload || {}));
      renderCritic(row);
      return row;
    } catch (e) {
      renderCritic({ ok: false, message: e.message || String(e) });
      return null;
    }
  }

  function initCriticObserver() {
    const check = $("btn-critic-check");
    if (check && check.dataset.criticWired !== "1") {
      check.dataset.criticWired = "1";
      check.addEventListener("click", function () {
        const text = ($("prompt-input") && $("prompt-input").value.trim()) || "";
        runCriticObserver(
          lastBuilderExchange || {
            text: text || "Review the current IDE state.",
            assistant_response: "",
            run_id: runId || "",
            quality_gate: lastQualityGate || null,
          },
        );
      });
    }
    const toBuilder = $("btn-critic-to-builder");
    if (toBuilder && toBuilder.dataset.criticWired !== "1") {
      toBuilder.dataset.criticWired = "1";
      toBuilder.addEventListener("click", function () {
        const next = ($("critic-next") && $("critic-next").textContent.trim()) || "";
        const input = $("prompt-input");
  ...
  function renderSiteIntelSummary(row) {
    const stats = $("site-intel-stats");
    const sites = $("site-intel-sites");
    const recent = $("site-intel-recent");
    if (!stats || !sites || !recent) return;
    const actions = (row && row.next_actions) || {};
    stats.innerHTML =
      "<span>Total " +
      esc((row && row.total) || 0) +
      "</span><span>FIX " +
      ...
      esc(actions.TASK || 0) +
      "</span><span>CRITIC " +
      esc(actions.CRITIC || 0) +
      "</span>";
    const siteRows = (row && row.sites) || [];
    sites.innerHTML = siteRows
      .map(function (s) {
        const risk = s.high ? "high" : s.medium ? "medium" : "low";
        return (
          '<div class="forge-site-intel-site"><strong>' +
          esc(s.site) +
          '<span class="forge-site-intel-risk-' +
          risk +
          '">' +
          esc(String(s.total || 0)) +
          " signals</span></strong><p>high " +
          esc(s.high || 0) +
          " · medium " +
          esc(s.medium || 0) +
          " · low " +
          esc(s.low || 0) +
          " · next " +
          esc(Object.keys(s.next_actions || {}).join(", ") || "MONITOR") +
          "</p></div>"
        );
      })
      .join("");
    const recentRows = ((row && row.recent) || []).slice(0, 6);
    recent.innerHTML = recentRows.length
      ? recentRows
          .map(function (ev) {
            return (
              '<div class="forge-site-intel-event"><strong>' +
              esc(ev.site || "site") +
              '<span class="forge-site-intel-risk-' +
              esc(ev.risk || "low") +
              '">' +
              esc(ev.next_action || "MONITOR") +
              "</span></strong><p>" +
              esc(ev.title || ev.text || "Signal") +
              "</p></div>"
            );
          })
          .join("")
      : '<div class="forge-site-intel-event"><p>No signals yet. Add feedback, chat notes, probes, or errors.</p></div>';
  }

  async function loadSiteIntelSummary() {
    try {
      const row = await api({ action: "site_intelligence_summary" });
      if (row.ok) renderSiteIntelSummary(row);
    } catch (e) {
      const recent = $("site-intel-recent");
      if (recent) {
        recent.innerHTML =
          '<div class="forge-site-intel-event"><p>Site Intelligence unavailable: ' + esc(e.message || e) + "</p></div>";
      }
    }
  }

  async function addSiteIntelSignal() {
    const site = $("site-intel-site");
    const kind = $("site-intel-kind");
    const text = $("site-intel-text");
    const message = (text && text.value.trim()) || "";
    if (!message) {
      setStatus("Add a site intelligence signal first");
      return;
    }
    try {
      const row = await api({
        action: "site_intelligence_ingest",
        site: (site && site.value) || "sourcea.app",
        kind: (kind && kind.value) || "feedback",
        text: message,
        source: "forge-private-admin",
      });
      if (!row.ok) throw new Error(row.message || row.error || "ingest failed");
      if (text) text.value = "";
      setStatus("Site signal added · " + ((row.event && row.event.next_action) || "MONITOR"));
      await loadSiteIntelSummary();
    } catch (e) {
      setStatus("Site signal failed");
      log(String(e.message || e), "err");
    }
  }

  function initSiteIntelligence() {
    const refresh = $("btn-site-intel-refresh");
    if (refresh && refresh.dataset.siteIntelWired !== "1") {
      refresh.dataset.siteIntelWired = "1";
      refresh.addEventListener("click", loadSiteIntelSummary);
    }
    const add = $("btn-site-intel-add");
    if (add && add.dataset.siteIntelWired !== "1") {
      add.dataset.siteIntelWired = "1";
      add.addEventListener("click", addSiteIntelSignal);
    }
    loadSiteIntelSummary();
  }
  ...
  function applyPills(row) {
    ...
    setLlmTether(llmOk ? "live" : "offline", llmOk ? "Model ready" : "Keys missing in ~/.sina/secrets.env");
    if (pillAnthropic) {
    ...
    if (row.desktop_mesh && !isSimpleIde()) applyMesh(row.desktop_mesh);
  }

  function applyMesh(mesh) {
    if (isSimpleIde()) return;
  ...
  function enableEditorWindow() {
    ...
    if (editor) {
      ...
      if (isEmbedScrollMode()) {
        editor.classList.remove("forge-movable-window");
  ...
  function initForgeToolsDeck() {
    ...
    if (!isSimpleIde()) renderMachinesDeck(null);
  }
  ...
  function renderMachinesDeck(mesh) {
    ...
    if (inChatUnifyShell && window.parent !== window) {
      const cuTiles = [
        { tab: "cu-ide", label: "Workspace", sub: "IDE · build", here: true },
  ...
  function sizeEditorPanes() {
    ...
    used += 20;
    const editorH = Math.max(480, editor.clientHeight || window.innerHeight - 80);
    const liveH = Math.max(280, editorH - used);
    if (live) {
    ...
    const embed = isEmbedScrollMode();
    if (embed) {
      const msgMin = Math.round(vh * 0.5);
      msgWin.style.setProperty("min-height", msgMin + "px", "important");
      msgOuterH = Math.max(msgMin, liveH - stripH - 12);
      msgWin.style.setProperty("height", msgOuterH + "px", "important");
      msgWin.style.setProperty("max-height", msgOuterH + "px", "important");
    }
    if (thread) {
      const threadH = Math.max(embed ? Math.round(vh * 0.42) : 160, msgOuterH - headH);
  ...
  function applyModels(row) {
    ...
    const saved = localStorage.getItem(MODEL_KEY);
    const wanted = saved || row.default_model || "gpt-4o";
    let html = "";
  ...
  function applySnapshot(row) {
    ...
    if (!isSimpleIde()) renderForgeToolsDeck(null, row);
  }
  ...
  function syncExecStripVisibility() {
    const strip = $("exec-strip");
    if (!strip) return;
    const hasTyping = !!document.querySelector("#chat-thread .forge-chat-typing");
    const realBubbles = document.querySelectorAll(
      "#chat-thread .forge-chat-bubble:not(.is-typing)",
    ).length;
    const embed = $("forge-app") && $("forge-app").classList.contains("forge-embed");
    const show = (inChatUnifyShell || embed) && realBubbles > 0 && !!runId && !hasTyping;
    if (show) {
      strip.hidden = false;
  ...
  async function tryZcpIngest(text) {
    if (!isZcpMessage(text)) return false;
    appendChatBubble("user", text, "plain");
    if ($("prompt-input")) {
      $("prompt-input").value = "";
      autoGrowPrompt();
      syncSendButtonState();
    ...
    setSendEnabled(false);
    setStatus("ZCP ingest…");
    const typing = appendChatBubble("typing", "ZCP bridge → parse · route · receipt", "plain");
    try {
      const row = await api({
        action: "zcp_ingest",
        input: text,
      ...
      const show =
        ...
        (row.zcp && row.zcp.mode
          ? "ZCP " + row.zcp.mode + " · " + (row.plane || "auto") + " · " + (row.task_id || "")
          : row.error || "ZCP failed");
      ...
      if (typing && typing.parentNode) typing.parentNode.removeChild(typing);
      appendChatBubble("assistant", "ZCP error: " + (e.message || e), "plain");
      setStatus("ZCP error");
      log(String(e.message || e), "err");
      return true;
    } finally {
  ...
  async function onRun() {
    const text = ($("prompt-input") && $("prompt-input").value.trim()) || "";
    if (!text) {
      setStatus("Enter a mission");
      return;
    }
    if (await tryZcpIngest(text)) {
      return;
    }
    const mode = getForgeMode();
    ...
    if (!llmOnline && $("opt-llm") && $("opt-llm").checked) {
      appendChatBubble(
        "assistant",
    ...
    if (inChatUnifyShell && agentId === "auto") {
      try {
    ...
    try {
      ...
      appendChatBubble("assistant", String(e.message || e), "plain");
      setLlmTether("error", String(e.message || e));
    ...
    try {
      const liveLlm = $("opt-llm") && $("opt-llm").checked;
      const chatMode = getForgeMode() === "chat" || isEmbedScrollMode() || isSimpleIde();
      const row = await apiChatTurn({
        action: "chat_turn",
        text: text,
        full_llm: liveLlm,
        fast: true,
        living_chat: chatMode,
        model: model,
        session_id: activeSessionId || undefined,
      });
      if (typing && typing.parentNode) typing.parentNode.removeChild(typing);
      ...
      const assistantText = displayAssistantText(row);
      ...
      lastBuilderExchange = {
        ...
        run_id: runId,
        quality_gate: row.quality_gate || (row.decision_card && row.decision_card.quality_gate) || null,
        living_chat: chatMode,
      };
      runCriticObserver(lastBuilderExchange);
      const meta = $("chat-meta");
      ...
      setLlmTether("live", ((row.llm && row.llm.model) || model) + " · reply ok");
      loadSnapshot();
      if (!isSimpleIde()) refreshMesh();
      loadChatSessions();
    } catch (e) {
      clearTypingBubble();
      appendChatBubble("assistant", String(e.message || e), "plain");
      setLlmTether("error", String(e.message || e).slice(0, 120));
  ...
  $("btn-run-composer") && $("btn-run-composer").addEventListener("click", onRun);
  $("btn-toggle-gate") &&
  ...
  $("prompt-input") && $("prompt-input").addEventListener("input", function () {
    autoGrowPrompt();
    updateComposerChars();
  });
  $("prompt-input") &&
    $("prompt-input").addEventListener("paste", function () {
      setTimeout(function () {
        autoGrowPrompt();
        updateComposerChars();
      }, 0);
  ...
  if (isSimpleIde()) {
    ...
    const goalLabel = document.querySelector(".forge-composer-label");
    if (goalLabel) goalLabel.textContent = "Message";
    const hint = $("composer-hint");
    if (hint) hint.textContent = "Enter to send · Shift+Enter new line · open folder to edit files";
    const prompt = $("prompt-input");
    if (prompt) {
      prompt.placeholder = "Type a message — Enter to send · Shift+Enter for new line";
    }
    setForgeMode("chat");
    const execStrip = $("exec-strip");
    if (execStrip) execStrip.hidden = true;
  }
  updateComposerChars();
  sanitizeSavedModel();
  restoreSavedModelSelect();
  ...
  if (inEmbed) {
    ...
    const scrollHint = $("composer-hint");
    if (scrollHint) {
      scrollHint.textContent = "Enter to send · Shift+Enter new line · scroll message history above";
    }
  ...
  updateFolderUI();
  setStatusBar();
  routeReady = initEngineRoute();
  bootReady = routeReady.then(function () {
    console.info("[Forge IDE] " + UI_VERSION + " · chat-live · living_chat fast path");
    loadAgentSelect();
    const msg = inChatUnifyShell
      ? CU_IDE_LABEL + " v" + UI_VERSION + " · combined Chat Unify upgrade"
      : isSimpleIde()
        ? "Forge IDE v" + UI_VERSION + " · workspace · chat · editor"
  ...
  if (draft && $("prompt-input") && !$("prompt-input").value) {
