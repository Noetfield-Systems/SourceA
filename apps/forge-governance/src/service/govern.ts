...
const DEFAULT_RATE_LIMIT = Number(
  process.env.FORGE_RATE_LIMIT_PER_MIN?.trim() || "60",
);
const DEFAULT_COST_LIMIT_USD = Number(
  process.env.FORGE_COST_LIMIT_USD?.trim() || "5",
);
...
function checkRateLimit(agentId: string): { status: "ALLOW" | "DENY"; reason?: string } {
  mkdirSync(join(getStateRoot(), "governance"), { recursive: true });
  const now = Date.now();
  let rows: Array<{ agent_id: string; at: number }> = [];
  const path = rateLimitFile();
  if (existsSync(path)) {
    try {
      rows = JSON.parse(readFileSync(path, "utf8")) as Array<{
        agent_id: string;
        at: number;
      }>;
    } catch {
...
function checkCostLimit(estimatedCost = 0.05): { status: "ALLOW" | "DENY"; reason?: string } {
  mkdirSync(join(getStateRoot(), "governance"), { recursive: true });
  const path = costFile();
  let total = 0;
  if (existsSync(path)) {
  ...
  writeFileSync(
    path,
    JSON.stringify({ total_usd: total + estimatedCost, updated_at: new Date().toISOString() }),
    "utf8",
  );
  return { status: "ALLOW", reason: "cost_limit_ok" };
}

function runKernel(request: GovernRequest): Promise<Record<string, unknown>> {
...
export async function governTask(request: GovernRequest): Promise<GovernDecision> {
  const checks: GovernDecision["checks"] = [];
  ...
  const allowed =
    kernel.status === "ALLOW"
      ? { status: "ALLOW" as const, reason: String(kernel.reason || "allowed_action_ok") }
      : { status: "DENY" as const, reason: String(kernel.reason || "allowed_action_denied") };
