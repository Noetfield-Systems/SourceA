import { createTaskWorker, saveReceipt, saveRunState, updateTaskStatus, } from "@sourcea/forge-core";
import { checkGovernance } from "../governance/client.js";
import { executePrompt } from "../executor/index.js";
async function persistFailure(task, error, governance) {
...
export function startQueueWorker() {
    ...
    worker.on("failed", async (job, error) => {
        const task = job?.data;
        console.error(`[forge-worker] failed task=${task?.id ?? "unknown"} error=${error.message}`);
        if (task?.id) {
            try {
