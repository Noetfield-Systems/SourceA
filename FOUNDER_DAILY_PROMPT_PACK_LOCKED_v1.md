...
| ID | Order (abbrev) |
|----|----------------|
| **007** | Execute one full Worker INBOX turn end-to-end. Read ~/.sina/worker-promp… |
| **019** | Daily health snapshot from disk JSON only — no hub color as truth. Repor… |
| **039** | Auto-heal routine: run hub_dual_heal_v1.py then machine_tune_pipeline_v1… |
| **053** | H1 Super Fast only: fetch worker-hub/v1 JSON — report task, queue, form … |
| **063** | Implement only the scope I attached or stated — match repo conventions, … |
| **081** | Run machine_test_ladder_run_v1.py --tier daily --json — paste steps_pass… |
| **093** | What should I do now? Give three disk-backed options with tradeoffs, one… |
| **002** | hospital — Run scripts/agent_hospital_pipeline_v1.py --role worker. Exec… |
| **071** | No fake progress: show validator stdout or receipt JSON — otherwise stat… |
| **098** | Session start ritual: rules-in-charge API, truth bundle JSON, session ga… |
