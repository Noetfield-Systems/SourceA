...
## L16 — Automation lane ownership (living system parallel)
...
- Duplicate execution of the **same action class** on the **same op_key** is forbidden (D1 + `data/automation-conflict-matrix-v1.json`).
- **GitHub Copilot** proposes PRs — **Worker** implements — **Brain** routes — **Cloud** runs motors — **GHA** proves truth.
- **GitHub Agents** use `repository_dispatch` with `{mission_id, workflow_id, op_key}` — motor class (`cloud_forge_run_auto_tick`, Railway motor drain) forbidden.
- **GHA** must not schedule the Cloud Forge Run motor (manual backup dispatch only); a GHA scheduled motor vs CF cron = P0 defect (conflict matrix C001).
- **Signal Factory** is piggyback-only on the loop-specialist tick (SA-T-signal-factory-piggyback); a dedicated `signal-factory-tick-v1` cron is forbidden (L14 sweep enforces).

**Governance SSOT:** `docs/GITHUB_AUTOMATION_LIVING_SYSTEM_GOVERNANCE_LOCKED_v1.md` · executors/lanes/conflicts in `data/github-automation-governance-v1.json` · validator `scripts/validate-github-automation-governance-v1.sh`.
...
| GitHub automation governance SSOT | `docs/GITHUB_AUTOMATION_LIVING_SYSTEM_GOVERNANCE_LOCKED_v1.md` |
| Executor / lane / conflict registries | `data/github-automation-governance-v1.json` · `data/automation-lane-registry-v1.json` · `data/automation-conflict-matrix-v1.json` |
| Governance validator | `scripts/validate-github-automation-governance-v1.sh` |
| Repository-dispatch handler (Agents) | `.github/workflows/repository-dispatch-v1.yml` |
