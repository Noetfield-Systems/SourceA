...
| H2 | Policy evaluation can run pre-commit without >500ms p95 overhead |  gate latency under load | p95 < 500ms on reference hardware | **Open** — not benchmarked |
...
-  policy engine under concurrent agent sessions  
...
| M4 | Latency  report | p50/p95 gate timing | `receipts/sred-experiment-log-2026/latency/` |
