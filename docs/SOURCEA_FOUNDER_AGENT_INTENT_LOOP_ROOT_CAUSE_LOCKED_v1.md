...
**Related:** `.cursor/rules/agent-founder-intent-first.mdc` · `.cursor/rules/033-market-analog-before-abstraction.mdc` · `data/agent-behavior-settings-v1.json` · `docs/COMPETITOR_ANALYSIS_FORMAT_MARKET_REALITY_v1.md` · `docs/FORGE_MVP_BLUEPRINT_LOCKED_v1.md`
**Incident thread:** Forge MVP + competitor-1000 + Terminal/404 (`POST http://…` → `zsh: command not found` → Hub `not found`)
...
| **Comparable competitors** (touchable startups) | ICP theory, GitHub/Stripe, abstract SourceA names | Several corrections |
...
|--------------|-------------------|
| Competitor / market | ICP, architecture, SourceA theory, big incumbents |
...
**Machine gap:** Rule 033 classifies competitor / ICP / implementation / company mapping — but nothing blocks an agent from starting WORK after a policy save.
...
- Step 9 “DONE”: `POST /api/fbe/forge-competitor-run/v1` without Hub button
...
- `forge_competitor_run_v1.py --dry-run` (local Python)

It does **not** check:

- HTTP POST to **running** Hub on `:13020`
- Hub Action in `agent-control-panel/command-data.json` (no `forge-competitor` entry at save time)
...
2. **Lane:** competitor | ICP | implementation | company mapping | build | explain
...
Align with: `docs/COMPETITOR_ANALYSIS_FORMAT_MARKET_REALITY_v1.md` (market answers). Mac Law = **agents route factory work to cloud**, not “no HTTP copy.”
...
- [ ] ASF asks for competitors → first reply has comparable companies + links + pricing evidence (no code unless asked)
...
| Topic | Path |
|-------|------|
| Market competitor format | `docs/COMPETITOR_ANALYSIS_FORMAT_MARKET_REALITY_v1.md` |
...
| Forge run script (works without Hub) | `scripts/forge_competitor_run_v1.py` |
...
| Validator scripts | New `validate-*` when ASF asked competitors |
...
| `market` | Competitors, pricing, who sells | Comparable vendors per `COMPETITOR_ANALYSIS_FORMAT_MARKET_REALITY_v1.md` | ICP theory, SourceA rename |
...
Rule 033 sub-lanes (`competitor` · `icp` · `implementation` · `company_mapping`) map under **`market`** and **`strategy`** — not under **`build`** unless unlocked.
