...
This batch moves **authority and portfolio truth** out of root — not incidents, not Sina Command legacy UI, not WORLD_TARGET_MODEL. Goal: establish `brain-os/law/` + `brain-os/system/` + `brain-os/entry/` as the read plane for structural questions.
...
| 2 | `SINA_GOVERNANCE_ENTRY_LOCKED_v1.md` | `brain-os/entry/` | move | **HIGH** — entry gate; update pointers in `.cursor/rules` if any cite root |
...
| 22 | `ASF_FULL_DAY_EXECUTION_PLAYBOOK_LOCKED_v1.md` | `brain-os/enforcement/` | move | low |
...
root/  →  brain-os/entry/        (1 file)    governance entry router
root/  →  brain-os/enforcement/  (1 file)    ASF full-day playbook
