...
Is **G3 Tailscale** proof recorded on disk? Gap vs hub **WIRE-G3** ops card and `WIRE_LANE_PROGRESS.md`?
...
1. **Wire preflight check** (`founder-wire-preflight`) — runs `npm run wire:preflight` in AI Dev Bridge OS  
   - Hint on disk: *"Code gates only — G3 Tailscale still needs iPhone proof"*
