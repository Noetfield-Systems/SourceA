...
| No partial blame closeout | INCIDENT-FIX-OWNERSHIP §0 | **FAIL** | **PASS** (law on disk) |
...
| `plan_only` latch blocks until `run inbox` | **On disk** | Worker |
...
> **Governed execution fails when law lives in documents but founder orders live in a corner of disk — unify SSOT, fail-closed on the hot path, propagate to every layer, or the system failed.**
