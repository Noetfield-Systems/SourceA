...
**Naming note:** Two different “015” subjects exist logged:
...
Run every step logged. Quote command output verbatim (tails OK). Classify each finding: CONDUCT | POISON | BOTH.
...
## Phase 4 — HISTORICAL sweep (find ALL existing conduct + poison logged)
...
### Table B — All poison loop artifacts still logged
...
Reply: Table A (conduct incidents logged) + Table B (poison artifacts) + count OPEN vs CLOSED
```
