...
```mermaid
flowchart TD
  REF[References locked] --> REC[Record Screen Studio]
  REC --> ING[Ingest + polish]
  ING --> CRIT[Critic vs pick ID]
  CRIT -->|PASS| SHIP[Ship gate publish_allowed]
  CRIT -->|BLOCK| LEARN[Learn row + rule patch]
  LEARN --> REC
  SHIP --> MEM[Memory incident]
  MEM --> REF
```
