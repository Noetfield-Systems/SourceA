...
**Product:** Trust on disk, visible in motion — not hype reels with fake UI.
...
| Founder style pick | ✅ STYLE-A3 (Vercel one-moment) + shot brief on disk |
| Reference catalog | ✅ 60+ URLs — `VIDEO_REFERENCE_CATALOG_FULL_2026-06-15_v1.md` |
| Reference → compile enforcement | 🟡 Partial — pick on disk; preflight not enforced yet |
...
1. Wire preflight in commercial_short_film_v1.py or ship_gate:
   ...
   - Require match_brief path exists on disk
...
Precondition: G100 PASS (SourceA master S/A on disk).
