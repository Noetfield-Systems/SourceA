...
| **Narrative** | ONE aha: **push → deploy → live** | ONE aha: **intent → gate → BLOCK → tamper verification built in** |
...
|-------------|-------------------|---------------|
| Developer **pushes** code | Agent **attempts** controlled action (ops / cross-lane) | W1 terminal `sourcea-boot --json` |
...
| **13.0–16.0** | Transition | Cursor to **TAMPER** tab · click (hard cut) | 1.2× follow cursor | 3s | `Receipt logged.` |
...
| **26.0–29.0** | Pull back | Zoom out to full W1 player · progress rail shows BLOCK+TAMPER done | 1.0× | 3s | `SourceA · controlled automation` |
...
| 13.0–16.0 | Receipt logged. |
| 16.0–21.0 | Tamper FAIL. |
| 21.0–26.0 | Audit preserved. Replay tomorrow. |
| 26.0–29.0 | SourceA · controlled automation |
...
- [ ] `commercial-short-demo.mp4` logged matches master
