...
Fix turn-bind drift: goal1-worker-turn-bind and headsup must match queue head sa on disk. Show before/after for every field that changed; minimal fix only — no unrelated edits. Done when bind JSON matches orchestrator expected sa and validate step for turn-bind passes.
...
Drain exactly one sa from plan-no-asf-run.sh output — pick the first eligible row, execute fully, then stop. No batch drain, no parallel sa work. Done when that single sa shows completed status on disk and you paste which sa_id was drained.
...
Two-hub sibling check: H1 worker-hub/v1 payload must stay small (<16KB); H2 machine-hub health must be fresh on disk. Confirm H2 is not nested inside H1 UI response. Done when sizes, mtimes, and sibling separation are cited.
...
Quarantine flags: read agent-maze-quarantine-v1.json and machine-forge-quarantine on disk. Report active true/false for each and what pipeline is blocked. Done when both flags cited with timestamps.
...
Fix the single highest P0 blocker only — minimal diff, one validator proof, then stop. Do not fix lower priority items in the same turn. Done when P0 id is closed on disk and validator PASS pasted.
...
Light refresh hub (mode light) then confirm H1 health pill is fresh on disk. Never full rebuild on founder Refresh — cite SUPER_FAST_HUB law if tempted. Done when refresh receipt or health mtime proves update.
...
Five-step SCAN→SAY→PICK→PROVE→SHIP: run SCAN on disk truth, SAY status in five lines, propose one PICK for Form, wait for founder — do not SHIP without PROVE. Founder clicks Form, not Terminal. Done when SCAN+SAY pasted and PICK row draft ready.
