...
|----------------------------|-------------|
| “Brain reads chat instead of disk” | **False.** Brain and Worker both read whatever the **system injects** — session gate · entry gate · active rules · broker cache · hub sync · `command-data` projection · stale JSON on disk. |
