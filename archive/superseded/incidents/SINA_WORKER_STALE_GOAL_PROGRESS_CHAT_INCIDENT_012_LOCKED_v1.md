# SUPERSEDED — use INCIDENT-013

**Saved:** 2026-06-10T08:41:55Z · **Retrofit:** doc-datetime-law batch retrofit
...
# Worker stale goal_progress chat report & false repeating steps (LOCKED)
...
**sequence_id:** SA-2026-06-10-INCIDENT-012  
**Classification:** MANDATORY READ — **every SourceA Worker** · auto-run executor · Brain · Maintainer · all Cursor executor agents  
**Canonical pointer:** `/Users/sinakazemnezhad/Desktop/SourceA/SINA_WORKER_STALE_GOAL_PROGRESS_CHAT_INCIDENT_012_REPORT_LOCKED_v1.md`  
...
**Related:** INCIDENT-006 (receipt law) · INCIDENT-007 (STALE broker / autorun) · INCIDENT-008 (stall/timing) · INCIDENT-009 (session closeout) · `AGENT_MISS_DISK_FIRST_CORRECTION_LOOP_LOCKED_v1.md`  
**Not this incident:** **INCIDENT-010** = CIR-COSPRO cross-lane SSOT edit (`SINA_CROSS_LANE_EDIT_FORBIDDEN_INCIDENT_LOCKED_v1.md`) · **INCIDENT-011** = REWRITE unauthorized disk edit  
**Duplicate warning:** A mistaken filing reused INCIDENT-010 id at `SINA_WORKER_STALE_GOAL_PROGRESS_CHAT_REPORT_INCIDENT_LOCKED_v1.md` — **do not cite**; this doc is the canonical **012** report. ASF cleanup via `EDIT ALLOWED` only.  
**Tags:** `INCIDENT-012`, `goal-progress`, `stale-report`, `healthy-drain`, `sa-0084-0093`, `worker-inbox`, `monitor-honesty`
...
| Metric (disk SSOT at lock time) | Value |
|-----------------------------------|------:|
...
### 3.5 Quinary: wrong incident id filed (governance)

An executor agent later filed this topic under **INCIDENT-010**, which is **already assigned** to CIR-COSPRO (cross-lane edit). That created a **duplicate id collision** on disk. **This report is INCIDENT-012** — use `sequence_id` SSOT, not reused numbers.
...
| Correct filing | **INCIDENT-012** locked in `brain-os/incidents/` (this doc) |
| Lock-time SSOT | **158/1000** · queue **19/30** · **sa-0090 CHECK** next |
...
- **Receipt law** — `receipts/sa-XXXX-receipt.json` written on VERIFY; `registry_honest_lib_v1` **no drift** at lock time.  
...
| 6 | Wrong **INCIDENT-010** id reused for this topic | Governance fragmentation — fixed by **012** |
...
- ❌ Reuse an existing **INCIDENT-NNN** id without checking compendium Part A
...
- ✅ New incidents → next free id in compendium · `brain-os/incidents/` + root pointer only
...
| Action | Owner | Status |
|--------|-------|--------|
| Stop parroting inject `goal_progress` | Worker agents | **Adopt §7–8** |
...
| Canonical **INCIDENT-012** locked (this doc) | Executor | **Done** |
| Remove mistaken INCIDENT-010 duplicate paths | ASF / Maintainer | **Open — EDIT ALLOWED only** |
| Hub TRACK BACKLOG sync lag | Sina Command / Maintainer | **Open** |
...
| `brain-os/incidents/SINA_ECOSYSTEM_INCIDENTS_AND_SESSION_INSIGHTS_COMPENDIUM_LOCKED_v1.md` | Incident id SSOT table (010 = CIR-COSPRO) |
| `brain-os/incidents/SINA_CROSS_LANE_EDIT_FORBIDDEN_INCIDENT_LOCKED_v1.md` | **INCIDENT-010** (not this report) |
| `SINA_REGISTRY_BATCH_FAKE_PROGRESS_INCIDENT_REPORT_LOCKED_v1.md` | INCIDENT-006 receipt law |
| `SINA_GOAL1_AUTORUN_BROKER_STALE_RECEIPT_INCIDENT_REPORT_LOCKED_v1.md` | INCIDENT-007 autorun / STALE broker |
| `SINA_WORKER_SESSION_MISTAKES_CLOSEOUT_INCIDENT_REPORT_LOCKED_v1.md` | INCIDENT-009 session closeout |
| `AGENT_MISS_DISK_FIRST_CORRECTION_LOOP_LOCKED_v1.md` | Miss correction law |

---

**END INCIDENT-012** — SA-2026-06-10-INCIDENT-012
