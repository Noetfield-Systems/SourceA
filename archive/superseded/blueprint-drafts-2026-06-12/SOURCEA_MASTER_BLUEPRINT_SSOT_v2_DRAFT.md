...
Answer: Unauditable AI actions. Ungoverned agent behavior. No receipt = no accountability = regulatory and operational liability. SourceA closes that gap deterministically.
...
| Canonical/Runtime split | Two projection files on disk | `agent-control-panel/command-data-canonical.json` · `command-data-runtime.json` |
...
4. Show the RT LIVE gate receipt on disk
