---
updated: 2026-06-30T12:38:39Z
lane: core
source_path: sites/SourceA-landing/green-unified/data/trust-signals.json
public: true
kind: json
---

# Trust Signals

- **schema**: sourcea-trust-signals-v1
- **at**: 2026-06-27T18:07:19Z
- **receipts_signed_today**: 4
- **receipts_signed_lifetime**: 5662
- **receipts_lifetime_label**: Logged and reviewable logged
- **receipts_date**: 2026-06-27
## receipt_samples
- agent-governance-events.jsonl:s10_eternal_audit
- agent-governance-events.jsonl:s10_eternal_audit
- agent-governance-events.jsonl:governance_drift
- critic-boot-v1.json
- **receipt_metric_label**: Material governance events today
- **valid_yes_total**: 1000
## github
- **ok**: False
- **repo**: sourcea-io/sourcea-boot
- **url**: https://github.com/sourcea-io/sourcea-boot
- **error**: HTTP Error 404: Not Found
## governance
- **loop**: governance
- **status**: operational
- **verdict**: PASS
- **checks_pass**: 4
- **checks_total**: 4
- **at**: 2026-06-24T03:23:04Z
- **factory**: FROZEN
- **factory_now_line**: factory-now · Cloud Workers.app owns Cloud Forge Run · head CLOUD-SEC-6967 · batch 71 · 100 rows/10min · 100 tasks/row
- **status_page**: /sourcea/status
- **proof_sample**: /sourcea/attach/proof-bundle-sample
## built_on
## temporal
- **id**: temporal
- **label**: Temporal
- **note**: durable workflow orchestration
## anthropic
- **id**: anthropic
- **label**: Anthropic Claude
- **note**: Claude API · policy at dispatch
## openai
- **id**: openai
- **label**: OpenAI
- **note**: API models · eval paths
## langgraph
- **id**: langgraph
- **label**: LangGraph
- **note**: graph orchestration pattern
## cursor
- **id**: cursor
- **label**: Cursor
- **note**: agentic worker IDE
## cloudflare
- **id**: cloudflare
- **label**: Cloudflare
- **note**: edge · tunnel · Pages
- **disclaimer**: Built on = technology dependencies · not co-marketing partnerships
## api_hook
- **endpoint**: POST /v1/decision
- **receipt_id**: CLOUD-SEC-6967
- **verdict**: PASS
- **signed_at**: 2026-06-24T03:23:04Z
