# wb-mkt-0052 — Delve · Buyer-visible UX
...
Write one-line UX spec: `Delve Buyer-visible UX` → buyer sees X → we show on `witnessbc.com pricing + toolkits hub` (lesson: Fast install motion — always verify independent auditor (lesson from 2026 controversy))
...
1. `status: done` in REGISTRY.json for `wb-mkt-0052`
