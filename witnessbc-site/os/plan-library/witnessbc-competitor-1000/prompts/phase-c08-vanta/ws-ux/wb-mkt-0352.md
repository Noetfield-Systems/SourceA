# wb-mkt-0352 — Vanta · Buyer-visible UX
...
Write one-line UX spec: `Vanta Buyer-visible UX` → buyer sees X → we show on `witnessbc.com pricing + toolkits hub` (lesson: Control → linked proof artifact (pattern only))
...
1. `status: done` in REGISTRY.json for `wb-mkt-0352`
