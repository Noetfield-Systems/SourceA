# wb-mkt-0002 — LowerPlane · Buyer-visible UX
...
Write one-line UX spec: `LowerPlane Buyer-visible UX` → buyer sees X → we show on `witnessbc.com pricing + toolkits hub` (lesson: Public pricing ladder like WitnessBC toolkits → install tiers)
...
1. `status: done` in REGISTRY.json for `wb-mkt-0002`
