# wb-mkt-0802 — Lasso Security · Buyer-visible UX
...
Write one-line UX spec: `Lasso Security Buyer-visible UX` → buyer sees X → we show on `witnessbc.com pricing + toolkits hub` (lesson: Shadow AI discovery in policy starter kits)
...
1. `status: done` in REGISTRY.json for `wb-mkt-0802`
