# wb-mkt-0952 — Maven AGI · Buyer-visible UX
...
Write one-line UX spec: `Maven AGI Buyer-visible UX` → buyer sees X → we show on `witnessbc.com pricing + toolkits hub` (lesson: Escalation + action log per ticket pattern)
...
1. `status: done` in REGISTRY.json for `wb-mkt-0952`
