# wb-mkt-0962 — Maven AGI · Pricing & packaging
...
Map Maven AGI revenue model (Annual enterprise + usage) to our `Free toolkits → Pro packs → Flow install → Ops retainer` tier names — no hidden fees theater
...
1. `status: done` in REGISTRY.json for `wb-mkt-0962`
