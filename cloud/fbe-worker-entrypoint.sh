#!/usr/bin/env bash
# FBE cloud worker entrypoint — W3 run-bay / assembly / full-job headless
set -euo pipefail
...
run_fleet() {
  local bay="${1:-sample-bay}"
  validate_skeleton
  echo "FBE W6 fleet run"
  python3 scripts/fbe_run_fleet_v1.py --json
  echo "OK: fbe-worker --run-fleet"
}
