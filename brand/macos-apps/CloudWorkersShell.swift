...
final class CloudWorkersLauncher {
    static let port = ProcessInfo.processInfo.environment["CLOUD_WORKERS_PORT"] ?? "13027"
    static var serverProcess: Process?

    static func bundleRoot() -> String {
        Bundle.main.bundlePath + "/Contents/Resources/cloud-workers-bundle"
    }
    ...
    static func logLine(_ msg: String) {
        ...
        let log = dir.appendingPathComponent("cloud-workers-app-launch.log")
    ...
    static func startServer(completion: @escaping (Bool) -> Void) {
        healthOK { ok in
            ...
            let script = bundleRoot() + "/scripts/cloud-workers-server.py"
            ...
            do {
                try proc.run()
                serverProcess = proc
                let pidFile = home.appendingPathComponent(".sina/cloud-workers-server.pid")
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    let port = CloudWorkersLauncher.port
    let appRouter = SinaAppRouter(homeApp: .cloud)

    func applicationDidFinishLaunching(_ notification: Notification) {
        SinaStandaloneShell.installStandardMenu(appName: "Cloud Workers")
        CloudWorkersLauncher.logLine("double-click launch")
        window = NSWindow(
        ...
        showLoading()
        CloudWorkersLauncher.startServer { ok in
    ...
    func showLoading() {
        let html = """
        <html><body style='background:#0a0f1a;color:#e8f0ff;font-family:system-ui;padding:48px;text-align:center'>
        <div style='font-size:2.5rem;margin-bottom:16px'>☁</div>
        <h1 style='font-weight:600'>Cloud Workers</h1>
        <p style='color:#7a8ba8'>Starting command center…</p>
        </body></html>
        """
    ...
    func showFailure() {
        let alert = NSAlert()
        alert.messageText = "Cloud Workers"
        alert.informativeText = "Could not start the server. Log: ~/.sina/cloud-workers-app-launch.log"
        alert.alertStyle = .warning
        alert.runModal()
        let html = """
        <html><body style='background:#0a0f1a;color:#e8f0ff;font-family:system-ui;padding:40px'>
        <h1>Cloud Workers</h1>
        <p>Server failed to start. Double-click again or check ~/.sina/cloud-workers-app-launch.log</p>
        <p>Worker Hub :13020 must be running for Proceed.</p>
        </body></html>
        """
        webView.loadHTMLString(html, baseURL: nil)
    }
        ...
        URLSession.shared.dataTask(with: healthURL) { _, resp, _ in
            DispatchQueue.main.async {
                if let http = resp as? HTTPURLResponse, http.statusCode == 200,
                   let pageURL = URL(string: "http://127.0.0.1:\(self.port)/?ui=1.0.0&t=\(Int(Date().timeIntervalSince1970))") {
                    CloudWorkersLauncher.logLine("window loading UI")
    ...
    func applicationWillTerminate(_ notification: Notification) {
        if let proc = CloudWorkersLauncher.serverProcess, proc.isRunning {
            proc.terminate()
        }
    }
}
