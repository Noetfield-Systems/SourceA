...
# mac_health_emergency_stop_v1.py is always a sibling of this file — both live
# in the same scripts/ directory whether running from the dev repo or from
# inside a built .app bundle, so resolve relative to our own location rather
# than any hardcoded personal path. SINA_SOURCEA (if set) still wins so a
# personal-edition SourceA checkout can override it.
_env_source_a = os.environ.get("SINA_SOURCEA", "").strip()
if _env_source_a and (Path(_env_source_a) / "scripts" / "mac-health-guard-server.py").is_file():
    SOURCE_A = Path(_env_source_a)
else:
    SOURCE_A = Path(__file__).resolve().parent.parent
SCRIPT = Path(__file__).resolve().parent / "mac_health_emergency_stop_v1.py"
DEBOUNCE_SEC = 2.0
_last = 0.0
