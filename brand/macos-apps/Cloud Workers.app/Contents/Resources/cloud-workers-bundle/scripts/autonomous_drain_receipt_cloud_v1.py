...
def observer_html(*, limit: int = 10) -> str:
    ...
    for c in payload.get("cycles") or []:
        ...
        ship = c.get("ship") or {}
        pack = c.get("pack") or {}
        proc = pack.get("processed", "—")
        adv = pack.get("advanced", "—")
        rows.append(
            f"<tr><td>{c.get('at','')}</td><td>{c.get('trigger_source','')}</td>"
            f"<td>{proc}/{pack.get('max_advance','100')}</td><td>{adv}</td>"
    ...
    body_rows = "\n".join(rows) or "<tr><td colspan=7>No cycles yet</td></tr>"
...
<table><thead><tr><th>At</th><th>Trigger</th><th>Processed</th><th>Shipped</th><th>PROVE</th><th>SHIP</th><th>Verdict</th></tr></thead>
